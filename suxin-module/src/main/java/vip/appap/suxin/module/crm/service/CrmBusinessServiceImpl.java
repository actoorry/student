...
import vip.appap.suxin.module.crm.controller.admin.vo.CrmBusinessPageReqVO;
import vip.appap.suxin.module.crm.controller.admin.vo.CrmBusinessSaveReqVO;
import vip.appap.suxin.module.crm.controller.admin.vo.CrmBusinessTransferReqVO;
import vip.appap.suxin.module.crm.controller.admin.vo.CrmBusinessUpdateStatusReqVO;
...
import vip.appap.suxin.module.crm.dal.mysql.CrmBusinessMapper;
import vip.appap.suxin.module.crm.dal.mysql.CrmBusinessProductMapper;
...
public class CrmBusinessServiceImpl implements CrmBusinessService {
    ...
    private List<CrmBusinessProductDO> validateBusinessProducts(List<CrmBusinessSaveReqVO.BusinessProduct> list) {
        // 1. 校验产品存在
        Set<Long> skuIds = convertSet(list, CrmBusinessSaveReqVO.BusinessProduct::getSkuId);
        ...
        return convertList(list, o -> BeanUtils.toBean(o, CrmBusinessProductDO.class,
                item -> item.setTotalPrice(
                        MoneyUtils.priceMultiply(new BigDecimal(item.getBusinessPrice()), item.getCount()).intValue())));
    }

    private void calculateTotalPrice(CrmBusinessDO business, List<CrmBusinessProductDO> businessProducts) {
        BigDecimal sum = businessProducts.stream()
                .map(p -> new BigDecimal(p.getTotalPrice()))
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        business.setTotalProductPrice(sum);
