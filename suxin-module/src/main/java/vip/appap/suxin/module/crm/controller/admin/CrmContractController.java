...
public class CrmContractController {
    ...
    private CrmContractRespVO buildContractDetail(CrmContractDO contract) {
        ...
        // 批量查询 SKU、SPU、单位信息
        Set<Long> skuIds = convertSet(businessProducts, CrmContractProductDO::getSkuId);
    ...
    private List<CrmContractRespVO> buildContractDetailList(List<CrmContractDO> contractList) {
        ...
        return BeanUtils.toBean(contractList, CrmContractRespVO.class, contractVO -> {
            // 2.1 设置客户信息
            findAndThen(customerPartnerMap, contractVO.getCustomerId(), partner -> contractVO.setCustomerName(partner.getName()));
            // 2.2 设置用户信息
            ...
            // 2.3 设置联系人信息
            findAndThen(contactMap, contractVO.getSignContactId(), contact -> {
                contractVO.setSignContactName(contact.getName());
            });
            // 2.4 设置商机信息
