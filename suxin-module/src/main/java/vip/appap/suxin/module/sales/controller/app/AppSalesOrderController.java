...
import vip.appap.suxin.module.sales.service.SalesPriceService;
...
@Validated
@Slf4j
public class AppSalesOrderController {
    ...
    public CommonResult<AppSalesOrderSettlementRespVO> settlementOrder(@Valid AppSalesOrderSettlementReqVO settlementReqVO) {
    ...
    @PermitAll
    public CommonResult<List<AppSalesProductSettlementRespVO>> settlementProduct(@RequestParam("spuIds") List<Long> spuIds) {
        return success(priceService.calculateProductPrice(getLoginUserId(), spuIds));
    }

    @PostMapping("/create")
    @Operation(summary = "创建订单")
    public CommonResult<AppSalesOrderCreateRespVO> createOrder(@Valid @RequestBody AppSalesOrderCreateReqVO createReqVO) {
        SalesOrderDO order = tradeOrderUpdateService.createOrder(getLoginUserId(), createReqVO);
        return success(new AppSalesOrderCreateRespVO().setId(order.getId()).setPayOrderId(order.getPayOrderId()));
    }
    ...
    public CommonResult<AppSalesOrderDetailRespVO> getOrderDetail(@RequestParam("id") Long id,
                                                                  @RequestParam(value = "sync", required = false) Boolean sync) {
        // 1.1 查询订单
        SalesOrderDO order = tradeOrderQueryService.getOrder(getLoginUserId(), id);
        if (order == null) {
            return success(null);
        }
        // 1.2 sync 仅在等待支付
        if (Boolean.TRUE.equals(sync)
                && SalesOrderStatusEnum.isUnpaid(order.getStatus()) && !order.getPayStatus()) {
        ...
        // 2.1 查询订单项
        List<SalesOrderItemDO> orderItems = tradeOrderQueryService.getOrderItemListByOrderId(order.getId());
        // 2.2 查询物流公司
        SalesDeliveryExpressDO express = order.getLogisticsId() != null && order.getLogisticsId() > 0 ?
                deliveryExpressService.getDeliveryExpress(order.getLogisticsId()) : null;
        // 2.3 最终组合
        return success(SalesOrderConvert.INSTANCE.convert02(order, orderItems, tradeOrderProperties, express));
    }

    @GetMapping("/get-express-track-list")
    @Operation(summary = "获得交易订单的物流轨迹")
    @Parameter(name = "id", description = "交易订单编号")
    public CommonResult<List<AppSalesOrderExpressTrackRespDTO>> getOrderExpressTrackList(@RequestParam("id") Long id) {
    ...
    public CommonResult<PageResult<AppSalesOrderPageItemRespVO>> getOrderPage(AppSalesOrderPageReqVO reqVO) {
        // 查询订单
        PageResult<SalesOrderDO> pageResult = tradeOrderQueryService.getOrderPage(getLoginUserId(), reqVO);
        // 查询订单项
        ...
        // 最终组合
        return success(SalesOrderConvert.INSTANCE.convertPage02(pageResult, orderItems));
    }

    @GetMapping("/activity-page")
    @Operation(summary = "获得我的活动分页")
    public CommonResult<PageResult<AppSalesOrderActivityRespVO>> getMyActivityPage(@Valid AppSalesOrderActivityPageReqVO reqVO) {
        return success(tradeOrderQueryService.getMyActivityPage(getLoginUserId(), reqVO));
    }
    ...
    public CommonResult<Boolean> receiveOrder(@RequestParam("id") Long id) {
        tradeOrderUpdateService.receiveOrderByPartner(getLoginUserId(), id);
        return success(true);
    }
    ...
    public CommonResult<Boolean> cancelOrder(@RequestParam("id") Long id) {
        tradeOrderUpdateService.cancelOrderByPartner(getLoginUserId(), id);
        return success(true);
    }
    ...
    public CommonResult<AppSalesOrderItemRespVO> getOrderItem(@RequestParam("id") Long id) {
        SalesOrderItemDO item = tradeOrderQueryService.getOrderItem(getLoginUserId(), id);
        return success(SalesOrderConvert.INSTANCE.convert03(item));
    }

    @PostMapping("/item/create-comment")
    @Operation(summary = "创建交易订单项的评价")
    public CommonResult<Long> createOrderItemComment(@RequestBody AppSalesOrderItemCommentCreateReqVO createReqVO) {
        return success(tradeOrderUpdateService.createOrderItemCommentByPartner(getLoginUserId(), createReqVO));
    }
