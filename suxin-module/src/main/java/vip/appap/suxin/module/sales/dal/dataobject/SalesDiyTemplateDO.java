 ...
 * 装修模板 DO
 *
 * 1. 新建一个模版，下面可以包含多个 {@link SalesDiyPageDO} 页面，例如说首页、我的
 * 2. 如果需要使用某个模版，则将 {@link #used} 设置为 true，表示已使用，有且仅有一个
 *
 * @author owen
 */
@TableName(value = "sales_diy_template", autoResultMap = true)
@KeySequence("sales_diy_template_seq") // 用于 Oracle、PostgreSQL、Kingbase、DB2、H2 数据库的主键自增。如果是 MySQL 等数据库，可不写。
@Data
...
@NoArgsConstructor
@AllArgsConstructor
public class SalesDiyTemplateDO extends BaseDO {
