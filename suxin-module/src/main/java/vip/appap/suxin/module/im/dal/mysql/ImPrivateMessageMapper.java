...
public interface ImPrivateMessageMapper extends BaseMapperX<ImPrivateMessageDO> {
    ...
    default Long selectUnreadCount(Long receiverId) {
        return selectCount(new LambdaQueryWrapperX<ImPrivateMessageDO>()
                .eq(ImPrivateMessageDO::getReceiverId, receiverId)
                .eq(ImPrivateMessageDO::getStatus, ImMessageStatusEnum.UNREAD.getStatus()));
    }

    /**
     * 查询用户的私信会话列表（使用 UNION ALL 优化性能）
     *
     * @param userId 当前用户编号
     ...
     * @return 会话列表
     */
    List<ImConversationBO> selectConversationList(@Param("userId") Long userId,
