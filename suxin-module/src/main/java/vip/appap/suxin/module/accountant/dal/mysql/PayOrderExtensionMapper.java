...
public interface PayOrderExtensionMapper extends BaseMapperX<PayOrderExtensionDO> {
    ...
    default List<PayOrderExtensionDO> selectListByChannelCodeAndWechatVirtualDeliverStatus(String channelCode,
                                                                                           Integer deliverStatus) {
        return selectList(new LambdaQueryWrapper<PayOrderExtensionDO>()
                .eq(PayOrderExtensionDO::getChannelCode, channelCode)
                .eq(PayOrderExtensionDO::getWechatVirtualDeliverStatus, deliverStatus));
    }

    default int updateWechatVirtualDeliveryById(Long id, PayOrderExtensionDO update) {
        return update(update, new LambdaQueryWrapper<PayOrderExtensionDO>()
                .eq(PayOrderExtensionDO::getId, id));
    }

    default int updateWechatVirtualDeliveryByIdAndStatus(Long id, Integer deliverStatus, PayOrderExtensionDO update) {
        return update(update, new LambdaQueryWrapper<PayOrderExtensionDO>()
                .eq(PayOrderExtensionDO::getId, id)
                .eq(PayOrderExtensionDO::getWechatVirtualDeliverStatus, deliverStatus));
    }
