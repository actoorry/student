...
import vip.appap.suxin.module.accountant.controller.admin.vo.PayOrderSubmitReqVO;
...
import vip.appap.suxin.module.accountant.enums.PayNotifyTypeEnum;
...
public class PayOrderServiceImpl implements PayOrderService {
    ...
    public PayOrderSubmitRespVO submitOrder(PayOrderSubmitReqVO reqVO, String userIp) {
        ...
        validateOrdinaryChannelNotUsedForVirtualGoods(order, reqVO.getChannelCode());
        // 1.32 校验支付渠道是否有效
    ...
    void validateOrderActuallyPaid(Long id) {
        ...
        orderExtensions.forEach(orderExtension -> {
            ...
            // 微信虚拟支付使用 /xpay/query_order 专用查询链路，不属于通用 PayClient。
            if (isWechatVirtualOrderExtension(orderExtension)) {
                return;
            }
    ...
    private void validateOrdinaryChannelNotUsedForVirtualGoods(PayOrderDO order, String channelCode) {
        if (PayChannelEnum.WX_VIRTUAL_LITE.getCode().equals(channelCode)) {
            return;
        }
        Long salesOrderId;
        try {
            salesOrderId = Long.valueOf(order.getMerchantOrderId());
        } catch (NumberFormatException ex) {
            return;
        }
        List<SalesOrderItemDO> items = salesOrderItemMapper.selectListByOrderId(salesOrderId);
        for (SalesOrderItemDO item : items) {
            ...
            if (spu != null && Boolean.TRUE.equals(spu.getIsWechatMiniappVirtualGoods())) {
                throw exception(WECHAT_VIRTUAL_PAY_ORDINARY_CHANNEL_FORBIDDEN);
            }
        }
    }
    ...
    private boolean syncOrder(PayOrderExtensionDO orderExtension) {
        // 微信虚拟支付由 WechatVirtualPayService 的专用补偿任务和结果查询负责同步。
        if (isWechatVirtualOrderExtension(orderExtension)) {
            return false;
        }
        try {
    ...
    private boolean expireOrder(PayOrderDO order) {
        try {
            ...
            for (PayOrderExtensionDO orderExtension : orderExtensions) {
                ...
                // 微信虚拟支付必须先走专用查询确认，不能使用通用 PayClient，也不能直接过期。
                if (isWechatVirtualOrderExtension(orderExtension)) {
                    return false;
                }
    ...
    private boolean isWechatVirtualOrderExtension(PayOrderExtensionDO orderExtension) {
        return Objects.equals(orderExtension.getChannelCode(), PayChannelEnum.WX_VIRTUAL_LITE.getCode());
    }
