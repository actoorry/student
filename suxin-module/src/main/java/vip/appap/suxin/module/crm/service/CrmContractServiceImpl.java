...
// import vip.appap.suxin.module.bpm.api.task.BpmProcessInstanceApi;
// import vip.appap.suxin.module.bpm.api.task.dto.BpmProcessInstanceCreateReqDTO;
import vip.appap.suxin.module.crm.controller.admin.vo.CrmContractPageReqVO;
import vip.appap.suxin.module.crm.controller.admin.vo.CrmContractSaveReqVO;
...
import vip.appap.suxin.module.crm.dal.dataobject.CrmContractProductDO;
import vip.appap.suxin.module.crm.dal.mysql.CrmContractMapper;
import vip.appap.suxin.module.crm.dal.mysql.CrmContractProductMapper;
import vip.appap.suxin.module.crm.dal.redis.no.CrmNoRedisDAO;
...
import static vip.appap.suxin.module.crm.util.CrmAuditStatusUtils.convertBpmResultToAuditStatus;
...
public class CrmContractServiceImpl implements CrmContractService {
    ...
    private void validateRelationDataExists(CrmContractSaveReqVO reqVO) {
        ...
        if (reqVO.getCustomerId() != null) {
            partnerService.validatePartner(reqVO.getCustomerId());
        }
        // 2. 校验负责人
    ...
    private List<CrmContractProductDO> validateContractProducts(List<CrmContractSaveReqVO.Product> list) {
        // 1. 校验产品存在
        Set<Long> skuIds = convertSet(list, CrmContractSaveReqVO.Product::getSkuId);
        ...
        return convertList(list, o -> BeanUtils.toBean(o, CrmContractProductDO.class,
                item -> item.setTotalPrice(
                        MoneyUtils.priceMultiply(new BigDecimal(item.getContractPrice()), item.getCount()).intValue())));
    }

    private void calculateTotalPrice(CrmContractDO contract, List<CrmContractProductDO> contractProducts) {
        BigDecimal sum = contractProducts.stream()
                .map(p -> new BigDecimal(p.getTotalPrice()))
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        contract.setTotalProductPrice(sum);
    ...
    public void submitContract(Long id, Long userId) {
        ...
        // 2. 创建合同审批流程实例
        // TODO bpm: 暂时移除 BPM 依赖，后续迁移到 suxin-module 后按需恢复
        String processInstanceId = null; // bpmProcessInstanceApi.createProcessInstance(userId, new BpmProcessInstanceCreateReqDTO()
