 ...
 */
@Data
public class SalesConfigBaseVO {
    ...
    @InEnum(value = SalesBrokerageEnabledConditionEnum.class, message = "分佣模式必须是 {value}")
    private Integer brokerageEnabledCondition;

    @Schema(description = "分销关系绑定模式", requiredMode = Schema.RequiredMode.REQUIRED, example = "0")
    @InEnum(value = SalesBrokerageBindModeEnum.class, message = "分销关系绑定模式必须是 {value}")
    private Integer brokerageBindMode;

    @Schema(description = "分销海报图地址数组", requiredMode = Schema.RequiredMode.REQUIRED, example = "[https://www.appap.vip/yudao.jpg]")
    ...
    @AssertTrue(message = "启用分销时必须完整填写分销配置")
    public boolean isBrokerageConfigValid() {
        return !Boolean.TRUE.equals(brokerageEnabled)
                || (brokerageEnabledCondition != null
                && brokerageBindMode != null
                && brokerageFirstPercent != null
                && brokerageSecondPercent != null
                && brokerageWithdrawMinPrice != null
                && brokerageWithdrawFeePercent != null
                && brokerageFrozenDays != null
                && brokerageWithdrawTypes != null
                && !brokerageWithdrawTypes.isEmpty());
    }
