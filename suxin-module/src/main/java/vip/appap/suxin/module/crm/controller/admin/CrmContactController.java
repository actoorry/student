...
import vip.appap.suxin.module.crm.service.CrmContactBusinessService;
import vip.appap.suxin.module.crm.service.CrmContactService;
...
public class CrmContactController {
    ...
    public CommonResult<List<CrmContactRespVO>> getSimpleContactList() {
        ...
        return success(convertList(list, contact -> {
            return new CrmContactRespVO().setId(contact.getId())
    ...
    private List<CrmContactRespVO> buildContactDetailList(List<CrmContactDO> contactList) {
        ...
        // 1.4 获取联系人自身的 partner 基础信息
        Map<Long, PartnerDO> contactPartnerMap = partnerService.getPartnerMap(
                convertList(contactList, CrmContactDO::getId));
        // 2. 转换成 VO
        return BeanUtils.toBean(contactList, CrmContactRespVO.class, contactVO -> {
            // 2.0 设置联系人 partner 基础信息
            PartnerDO contactPartner = contactPartnerMap.get(contactVO.getId());
            if (contactPartner != null) {
                contactVO.setMobile(contactPartner.getMobile());
                contactVO.setEmail(contactPartner.getEmail());
                contactVO.setAreaId(contactPartner.getAreaId() != null ? contactPartner.getAreaId().intValue() : null);
                contactVO.setDetailAddress(contactPartner.getDetailAddress());
                contactVO.setSex(contactPartner.getSex());
                contactVO.setRemark(contactPartner.getRemark());
            }
            contactVO.setAreaName(AreaUtils.format(contactVO.getAreaId()));
            // 2.1 设置客户名称
            MapUtils.findAndThen(customerPartnerMap, contactVO.getCustomerId(), partner -> contactVO.setCustomerName(partner.getName()));
            // 2.2 设置创建人、负责人名称
            ...
            // 2.3 设置直属上级名称
            CrmContactDO parentContact = parentContactMap.get(contactVO.getParentId());
            if (parentContact != null) {
                contactVO.setParentName(parentContact.getName());
            }
        });
    }
