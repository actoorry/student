...
@AllArgsConstructor
@Getter
public enum SalesCombinationRecordStatusEnum implements ArrayValuable<Integer> {

    IN_PROGRESS(0, "进行中"),
    SUCCESS(1, "拼团成功"),
    FAILED(2, "拼团失败");

    public static final Integer[] ARRAYS = Arrays.stream(values()).map(SalesCombinationRecordStatusEnum::getStatus).toArray(Integer[]::new);
