...
public interface ProductSkuMapper extends BaseMapperX<ProductSkuDO> {
    ...
    default ProductSkuDO selectByWechatVirtualProductId(String wechatVirtualProductId) {
        return selectOne(ProductSkuDO::getWechatVirtualProductId, wechatVirtualProductId);
    }
    ...
    default int updateWechatVirtualGoodsFields(Long id, ProductSkuDO update) {
        return update(update, new LambdaUpdateWrapper<ProductSkuDO>().eq(ProductSkuDO::getId, id));
    }

    /**
     * 更新微信虚拟道具字段，并显式清空历史失败原因。
     *
     * MyBatis-Plus 默认忽略实体中的 null 字段，不能依赖
     * {@code setWechatVirtualReviewFailReason(null)} 生成 SET NULL。
     */
    default int updateWechatVirtualGoodsFieldsAndClearReviewFailReason(Long id, ProductSkuDO update) {
        return update(update, new LambdaUpdateWrapper<ProductSkuDO>()
                .set(ProductSkuDO::getWechatVirtualReviewFailReason, null)
                .eq(ProductSkuDO::getId, id));
    }
