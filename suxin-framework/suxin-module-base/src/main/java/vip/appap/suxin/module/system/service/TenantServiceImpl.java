...
import vip.appap.suxin.framework.tenant.config.TenantProperties;
...
import vip.appap.suxin.module.system.controller.admin.vo.RoleSaveReqVO;
import vip.appap.suxin.module.system.controller.admin.vo.TenantPageReqVO;
import vip.appap.suxin.module.system.controller.admin.vo.TenantSaveReqVO;
import vip.appap.suxin.module.system.convert.TenantConvert;
...
import vip.appap.suxin.module.system.dal.dataobject.TenantPackageDO;
import vip.appap.suxin.module.system.dal.mysql.TenantMapper;
import vip.appap.suxin.module.system.enums.RoleCodeEnum;
import vip.appap.suxin.module.system.enums.RoleTypeEnum;
...
public class TenantServiceImpl implements TenantService {
    ...
    @Autowired(required = false) // 由于 suxin.tenant.enable 配置项，可以关闭多租户的功能，所以这里只能不强制注入
    ...
    public Long createTenant(TenantSaveReqVO createReqVO) {
        ...
        // 创建租户对应的客商（共享主键：租户 id = 客商 id），复用联系人/联系手机作为客商名称/手机号
        Long partnerId = createPartner(createReqVO);

        // 创建租户，ID = partnerId（显式指定，不自增）
        TenantDO tenant = BeanUtils.toBean(createReqVO, TenantDO.class);
        tenant.setId(partnerId);
        ...
        TenantUtils.execute(tenant.getId(), () -> {
            ...
            // 创建用户，并分配角色
            Long userId = createUser(roleId, partnerId, createReqVO);
            // 修改租户的管理员
     ...
     * 创建租户对应的客商
     *
     * 复用联系人/联系手机作为客商名称/手机号；新建客商的自增 id 作为共享主键
     * （客商 id = 租户 id = 管理员用户 id），参与当前事务，失败自动回滚
     */
    private Long createPartner(TenantSaveReqVO createReqVO) {
        PartnerCreateReqVO reqVO = new PartnerCreateReqVO();
        reqVO.setName(createReqVO.getContactName());
        reqVO.setNickname(createReqVO.getContactName());
        reqVO.setMobile(createReqVO.getContactMobile());
        return partnerUserService.createPartner(reqVO);
    }

    private Long createUser(Long roleId, Long partnerId, TenantSaveReqVO createReqVO) {
        // 创建用户
        Long userId = userService.createUser(TenantConvert.INSTANCE.convert02(createReqVO, partnerId));
        // 分配角色
