...
public class UserController {
    ...
    public CommonResult<PageResult<UserRespVO>> getUserPage(@Valid UserPageReqVO pageReqVO) {
        ...
        // 获取关联的 partner 信息（使用 user.id = partner.id）
        Set<Long> userIds = convertSet(pageResult.getList(), AdminUserDO::getId);
        Map<Long, PartnerDO> partnerMap = partnerUserService.getPartnerMap(userIds);
        // 获取部门信息
        Map<Long, DeptDO> deptMap = deptService.getDeptMap(
                convertList(pageResult.getList(), AdminUserDO::getDeptId));
        // 转换并填充数据
        return success(new PageResult<>(UserConvert.INSTANCE.convertList(pageResult.getList(), deptMap, partnerMap),
                pageResult.getTotal()));
    }
    ...
    public CommonResult<List<UserSimpleRespVO>> getSimpleUserList() {
        ...
        Map<Long, PartnerDO> partnerMap = partnerUserService.getPartnerMap(userIds);
        return success(UserConvert.INSTANCE.convertSimpleList(list, deptMap, partnerMap));
    }
    ...
    public CommonResult<UserRespVO> getUser(@RequestParam("id") Long id) {
        ...
        return success(UserConvert.INSTANCE.convert(user, dept, partner));
    }
    ...
    public void exportUserList(@Validated UserPageReqVO exportReqVO,
        ...
        ExcelUtils.write(response, "用户数据.xls", "数据", UserRespVO.class,
                UserConvert.INSTANCE.convertList(list, deptMap, partnerMap));
    }
