...
import vip.appap.suxin.module.system.mq.message.mail.MailSendMessage;
import vip.appap.suxin.module.system.mq.producer.mail.MailProducer;
...
public class MailSendServiceImpl implements MailSendService {
    ...
    private String getUserMail(Long userId, Integer userType) {
        ...
        if (UserTypeEnum.ADMIN.getValue().equals(userType)) {
            // 邮箱已迁移到 partner 表（user.id = partner.id）
        ...
        if (UserTypeEnum.MEMBER.getValue().equals(userType)) {
            return partnerUserService.getMemberUserEmail(userId);
        }
        return null;
    }
