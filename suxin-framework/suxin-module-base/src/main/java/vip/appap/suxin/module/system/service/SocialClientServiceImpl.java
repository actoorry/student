...
import vip.appap.suxin.framework.common.util.cache.CacheUtils;
import vip.appap.suxin.framework.common.util.http.HttpUtils;
...
import vip.appap.suxin.module.system.dal.redis.RedisKeyConstants;
import vip.appap.suxin.module.system.enums.SocialTypeEnum;
import vip.appap.suxin.module.system.framework.justauth.core.AuthRequestFactory;
...
import static vip.appap.suxin.framework.common.util.date.LocalDateTimeUtils.UTC_MS_WITH_XXX_OFFSET_FORMATTER;
import static vip.appap.suxin.framework.common.util.date.LocalDateTimeUtils.toEpochSecond;
...
public class SocialClientServiceImpl implements SocialClientService {
     ...
     * 3. developer：开发版
     */
    @Value("${suxin.wxa-code.env-version:release}")
     ...
     * 3. formal：正式版
     */
    @Value("${suxin.wxa-subscribe-message.miniprogram-state:formal}")
    ...
    @Override
    public WxMaJscode2SessionResult getWxMaSessionInfo(Integer userType, String code) {
        WxMaService service = getWxMaService(userType);
        try {
            return service.getUserService().getSessionInfo(code);
        } catch (WxErrorException e) {
            log.error("[getWxMaSessionInfo][userType({}) code({}) 鑾峰緱浼氳瘽淇℃伅澶辫触]", userType, code, e);
            throw exception(SOCIAL_USER_AUTH_FAILURE, e.getError().getErrorMsg());
        }
    }
    ...
    @Override
    public String getWxMaAccessToken(Integer userType) {
        return getWxMaAccessToken(userType, false);
    }

    @Override
    public String getWxMaAccessToken(Integer userType, boolean forceRefresh) {
        WxMaService service = getWxMaService(userType);
        try {
            return service.getAccessToken(forceRefresh);
        } catch (WxErrorException e) {
            log.error("[getWxMaAccessToken][userType({}), forceRefresh({}) 获得小程序 accessToken 失败]",
                    userType, forceRefresh, e);
            throw exception(SOCIAL_USER_AUTH_FAILURE, e.getError().getErrorMsg());
        }
    }
    ...
    @Override
    public String mediaCheckAsync(Integer userType, String openid, String mediaUrl, Integer mediaType, Integer scene) {
        WxMaService service = getWxMaService(userType);
        try {
            // 构建请求参数
            ...
            // 调用微信接口
            String result = service.post("https://api.weixin.qq.com/wxa/media_check_async", jsonObject.toString());
            JsonObject resultJson = JsonParser.parseString(result).getAsJsonObject();

            // 检查错误码
            int errcode = resultJson.get("errcode").getAsInt();
            if (errcode != 0) {
                String errmsg = resultJson.get("errmsg").getAsString();
                log.error("[mediaCheckAsync][调用微信内容安全接口失败：openid({}) mediaUrl({}) errcode({}) errmsg({})]",
                        openid, mediaUrl, errcode, errmsg);
                throw exception(SOCIAL_CLIENT_WEIXIN_MINI_APP_MEDIA_CHECK_ERROR, errmsg);
            }

            // 返回 trace_id
            String traceId = resultJson.get("trace_id").getAsString();
            log.info("[mediaCheckAsync][调用微信内容安全接口成功：openid({}) mediaUrl({}) traceId({})]",
                    openid, mediaUrl, traceId);
            return traceId;
        } catch (WxErrorException e) {
            log.error("[mediaCheckAsync][调用微信内容安全接口异常：openid({}) mediaUrl({})]", openid, mediaUrl, e);
            throw exception(SOCIAL_CLIENT_WEIXIN_MINI_APP_MEDIA_CHECK_ERROR, e.getError().getErrorMsg());
        }
    }
    ...
    WxMaService getWxMaService(Integer userType) {
        ...
        log.info("[getWxMaService][userType={}, tenantId={}]", userType, tenantId);
        SocialClientDO client = socialClientMapper.selectBySocialTypeAndUserType(
                SocialTypeEnum.WECHAT_MINI_PROGRAM.getType(), userType);
        log.info("[getWxMaService][查询结果: client={}]", client != null ? client.getClientId() : "null");
        if (client != null && Objects.equals(client.getStatus(), CommonStatusEnum.ENABLE.getStatus())) {
