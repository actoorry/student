package vip.appap.suxin.module.infra.service.codegen.inner;
...
import vip.appap.suxin.framework.apilog.core.enums.OperateTypeEnum;
...
import vip.appap.suxin.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import vip.appap.suxin.module.infra.dal.dataobject.codegen.CodegenTableDO;
import vip.appap.suxin.module.infra.enums.codegen.CodegenFrontTypeEnum;
import vip.appap.suxin.module.infra.enums.codegen.CodegenSceneEnum;
import vip.appap.suxin.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import vip.appap.suxin.module.infra.enums.codegen.CodegenVOTypeEnum;
import vip.appap.suxin.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 suxin-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 suxin-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("vip.appap.suxin.module.base.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/suxin-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/suxin-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "suxin-module-${table.moduleName}/" + // 顶级模块
                "suxin-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "suxin-module-${table.moduleName}/" + // 顶级模块
                "suxin-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "suxin-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "suxin-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "suxin-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "suxin-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
