package vip.appap.suxin.framework.web.core.handler;
...
import vip.appap.suxin.framework.common.exception.util.ServiceExceptionUtil;
...
import static vip.appap.suxin.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    public CommonResult<?> allExceptionHandler(HttpServletRequest request, Throwable ex) {
        ...
        CommonResult<?> dataIntegrityResult = handleDataIntegrityViolation(ex);
        if (dataIntegrityResult != null) {
    ...
    public CommonResult<?> methodArgumentTypeInvalidFormatExceptionHandler(HttpMessageNotReadableException ex) {
        ...
        // 兜底识别 JsonMappingException（如 LocalDate 字段收到毫秒时间戳），提示风格与 InvalidFormatException 分支保持一致
            ...
            if (StrUtil.isNotEmpty(msg)) {
                ...
                if (idx > 0 && idx < msg.length() - 1) {
                    return CommonResult.error(BAD_REQUEST.getCode(),
                            String.format("请求参数类型错误:%s", msg.substring(idx + 1).trim()));
                }
            }
        }
    ...
    public CommonResult<?> sqlIntegrityExceptionHandler(SQLIntegrityConstraintViolationException ex) {
        log.warn("[sqlIntegrityExceptionHandler]", ex);
        return CommonResult.error(BAD_REQUEST.getCode(), "数据已存在，请勿重复操作");
    }

    /**
     * 处理字段超长、非空约束等数据完整性异常（兜底，优先依赖 VO 校验拦截）
     * web 模块不依赖 spring-jdbc，按异常类名识别 DataIntegrityViolationException
     */
    private CommonResult<?> handleDataIntegrityViolation(Throwable ex) {
        if (!isDataIntegrityViolation(ex)) {
            return null;
        }
        log.warn("[handleDataIntegrityViolation]", ex);
        String rootMessage = ExceptionUtil.getRootCauseMessage(ex);
        if (StrUtil.containsIgnoreCase(rootMessage, "Data too long")) {
            if (StrUtil.containsIgnoreCase(rootMessage, "employee_no")) {
                return CommonResult.error(BAD_REQUEST.getCode(), "员工工号长度超过限制，请缩短后重试");
            }
            if (StrUtil.containsIgnoreCase(rootMessage, "employee_mobile")) {
                return CommonResult.error(BAD_REQUEST.getCode(), "员工手机号长度超过限制，请检查后重试");
            }
            return CommonResult.error(BAD_REQUEST.getCode(), "输入内容过长，请缩短后重试");
        }
        return CommonResult.error(BAD_REQUEST.getCode(), "数据保存失败，请检查输入是否符合要求");
    }

    private static boolean isDataIntegrityViolation(Throwable ex) {
        for (Throwable current = ex; current != null; current = current.getCause()) {
            if ("org.springframework.dao.DataIntegrityViolationException".equals(current.getClass().getName())) {
                return true;
            }
        }
        return false;
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块 suxin-module-report - 表结构未导入][参考 https://cloud.iocoder.cn/report/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[报表模块 suxin-module-report - 表结构未导入][参考 https://cloud.iocoder.cn/report/ 开启]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 suxin-module-bpm - 表结构未导入][参考 https://cloud.iocoder.cn/bpm/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[工作流模块 suxin-module-bpm - 表结构未导入][参考 https://cloud.iocoder.cn/bpm/ 开启]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号 suxin-module-mp - 表结构未导入][参考 https://cloud.iocoder.cn/mp/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[微信公众号 suxin-module-mp - 表结构未导入][参考 https://cloud.iocoder.cn/mp/build/ 开启]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统 suxin-module-mall - 已禁用][参考 https://cloud.iocoder.cn/mall/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[商城系统 suxin-module-mall - 已禁用][参考 https://cloud.iocoder.cn/mall/build/ 开启]");
        }
        // 5. ERP 系统
        if (message.contains("erp_")) {
            log.error("[ERP 系统 suxin-module-erp - 表结构未导入][参考 https://cloud.iocoder.cn/erp/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[ERP 系统 suxin-module-erp - 表结构未导入][参考 https://cloud.iocoder.cn/erp/build/ 开启]");
        }
        // 6. CRM 系统
        if (message.contains("crm_")) {
            log.error("[CRM 系统 suxin-module-crm - 表结构未导入][参考 https://cloud.iocoder.cn/crm/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[CRM 系统 suxin-module-crm - 表结构未导入][参考 https://cloud.iocoder.cn/crm/build/ 开启]");
        }
        // 7. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块 suxin-module-pay - 表结构未导入][参考 https://cloud.iocoder.cn/pay/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[支付模块 suxin-module-pay - 表结构未导入][参考 https://cloud.iocoder.cn/pay/build/ 开启]");
        }
        // 8. AI 大模型
        if (message.contains("ai_")) {
            log.error("[AI 大模型 suxin-module-ai - 表结构未导入][参考 https://cloud.iocoder.cn/ai/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[AI 大模型 suxin-module-ai - 表结构未导入][参考 https://cloud.iocoder.cn/ai/build/ 开启]");
        }
        // 9. IoT 物联网
        if (message.contains("iot_")) {
            log.error("[IoT 物联网 suxin-module-iot - 表结构未导入][参考 https://doc.appap.vip/iot/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IoT 物联网 suxin-module-iot - 表结构未导入][参考 https://doc.appap.vip/iot/build/ 开启]");
        }
        return null;
    }
