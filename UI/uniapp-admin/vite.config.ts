...
/** 芋道 UI 组件解析器：本地 @/components/suxin-ui/* 的组件 */
function SuxinUiResolver(): ComponentResolver {
  return {
    type: 'component',
    resolve: (name: string) => {
      if (name.match(/^Yd[A-Z]/)) {
        ...
        return {
          name,
          from: `@/components/suxin-ui/${compName}/${compName}.vue`,
        }
      }
    },
  }
...
export default defineConfig(({ command, mode }) => {
  ...
  return defineConfig({
    ...
    plugins: [
      ...
      UniManifest(),
      UniComponents({
        resolvers: [WotResolver(), SuxinUiResolver()],
        extensions: ['vue'],
        deep: true, // 是否递归扫描子目录，
