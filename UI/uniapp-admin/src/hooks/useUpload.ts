...
import { prepareUploadPath } from '@/utils/uploadFile'
...
export default function useUpload<T extends TfileType>(options: TOptions<T> = {} as TOptions<T>) {
  ...
  const handleFileChoose = async ({ tempFilePath, size }: { tempFilePath: string, size: number }) => {
    if (size > maxSize) {
    ...
    loading.value = true
    // 图片类型上传前统一压缩预处理（默认开启）
    const preparedPath = fileType === 'image'
      ? await prepareUploadPath(tempFilePath, '', 'image', true)
      : tempFilePath
    uploadFile({
  ...
  const run = () => {
    ...
    const chooseFileOptions = {
      ...
      success: async (res: any) => {
        ...
        size = res.tempFiles[0].size
        // #endif
        await handleFileChoose({ tempFilePath, size })
      },
      fail: (err: any) => {
