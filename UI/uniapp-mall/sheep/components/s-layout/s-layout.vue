<template>
  ...
  >
    <view class="page-main" :style="[bgMain]">
      ...
      <s-custom-navbar
        v-else-if="navbar === 'custom' && navbarEnabled && navbarMode === 'normal'"
        :data="navbarStyle"
      ...
      <view class="page-body" :style="[bgBody]">
        ...
        <s-custom-navbar
          v-if="navbar === 'custom' && navbarEnabled && navbarMode === 'inner'"
          :data="navbarStyle"
          :showLeftButton="showLeftButton"
        />
...
<script setup>
  ...
  // 历史装修数据缺少 enabled 时继续显示；只有显式 false 才关闭装修导航栏
  const navbarEnabled = computed(() => props.navbarStyle?.enabled !== false);
