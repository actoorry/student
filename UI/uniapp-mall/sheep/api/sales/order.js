...
const OrderApi = {
  ...
  settlementOrder: (data) => {
    ...
    if (![0, 1, 2, 3].includes(Number(data.deliveryType))) {
    ...
    const queryString = Object.keys(data2)
      ...
      .join('&');
    return request({
      url: `/sales/order/settlement?${queryString}`,
      method: 'GET',
      custom: {
        showError: true,
        showLoading: true,
  ...
  getSettlementProduct: (spuIds) => {
    return request({
      url: '/sales/order/settlement-product',
      method: 'GET',
      params: { spuIds },
      custom: {
        showLoading: false,
  ...
  getOrderExpressTrackList: (id) => {
    return request({
      url: `/sales/order/get-express-track-list`,
      method: 'GET',
      params: {
        id,
      },
  ...
  createOrderItemComment: (data) => {
    return request({
      url: `/sales/order/item/create-comment`,
      method: 'POST',
      data,
    });
  },
