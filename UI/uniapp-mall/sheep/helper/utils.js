 ...
 * @param {*} rootId 根Id 默认 0（当无法自动识别根节点时使用）
 */
export function handleTree(
  ...
  // 收集所有的 parentId
  const parentIds = new Set(cloneData.map(item => item[parentId]));
  
  // 找出根节点：parentId 不在任何节点的 id 中，或者等于指定的 rootId
  const rootNodes = cloneData.filter(item => {
    // 如果 parentId 不在任何节点的 id 中，说明是根节点
    const isRoot = !itemMap.has(item[parentId]);
    // 或者 parentId 等于指定的 rootId
    const isSpecifiedRoot = item[parentId] === rootId;
    return isRoot || isSpecifiedRoot;
  });
