...
// 登录后目标恢复：未登录访问受保护页面时保存完整跳转目标，登录成功后自动恢复。
const AUTH_REDIRECT_KEY = 'auth-redirect-url';
const saveAuthRedirect = (url) => {
  if (!url) return;
  uni.setStorageSync(AUTH_REDIRECT_KEY, url);
};
export const getAuthRedirectUrl = () => uni.getStorageSync(AUTH_REDIRECT_KEY) || '';
export const clearAuthRedirect = () => uni.removeStorageSync(AUTH_REDIRECT_KEY);
...
const _go = (
  ...
  // 页面登录拦截：未登录访问受保护页面时保存目标并引导统一登录，登录成功后恢复
  if (nextRoute.meta?.auth && !$store('user').isLogin) {
    saveAuthRedirect(url);
    showAuthModal();
    return;
  }
