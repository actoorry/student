            ...
            <template #tool>
              <view class="ss-flex">
                <button
                  ...
                  v-if="
                    [10, 20, 30].includes(state.orderInfo.status) &&
                    item.afterSaleStatus === 0 &&
                    state.orderInfo.payChannelCode !== 'wx_virtual_lite'
                  "
                  @tap.stop="
                ...
                <text
                  v-if="
                    ...
                    item.afterSaleStatus === 0
                  "
                  class="ss-font-24 text-disabled"
                >
                  虚拟支付争议请联系客服人工处理
                </text>
                <button
...
<script setup>
  ...
  import DeliveryApi from '@/sheep/api/sales/delivery';
  ...
  function onPay(payOrderId) {
    sheep.$router.go('/pages/accountant/index', {
      id: payOrderId,
    });
  }
  ...
  onLoad(async (options) => {
    ...
    if (!id && options.payOrderNo) {
      // 查询支付订单：根据 payOrderNo 取 merchantOrderId，merchantOrderId 即 salesOrderId
