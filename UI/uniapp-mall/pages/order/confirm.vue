<template>
  <s-layout title="确认订单" showLeftButton>
    <!-- 头部地址选择【配送地址】【自提地址】 -->
    <AddressSelection
      v-model="addressState"
      :available-types="state.orderInfo.availableDeliveryTypes || []"
    />
...
<script setup>
  ...
  // 提交订单
  function onConfirm() {
    if (![0, 1, 2, 3].includes(addressState.value.deliveryType)) {
  ...
  async function submitOrder() {
    ...
    // 更新购物车列表，如果来自购物车
    if (state.orderPayload.items.some((item) => item.cartId > 0)) {
      sheep.$store('cart').getList();
    }

    // 跳转到支付页面
    if (data.payOrderId && data.payOrderId > 0) {
      sheep.$router.redirect('/pages/accountant/index', {
        id: data.payOrderId,
      });
    } else {
  ...
  async function getOrderInfo() {
    ...
    const availableTypes = data.availableDeliveryTypes || [];
    if (!availableTypes.includes(addressState.value.deliveryType)) {
      addressState.value.deliveryType = availableTypes.length === 1 ? availableTypes[0] : undefined;
    }
  ...
  onLoad(async (options) => {
    ...
    // 获取交易配置
    const { data, code } = await SalesConfigApi.getSalesConfig();
    if (code === 0) {
      addressState.value.isPickUp = data.deliveryPickUpEnabled;
    }

    // 先由后端返回全部订单项的共同配送方式；仅有一个时自动选择。
