...
<script setup>
  ...
  const state = reactive({
    ...
    keyword: '',
    categorySales: 0,
    isMilitary: false, // 军创区上下文：true 时分页请求额外携带 isMilitary=true
    tabList: [
      {
        name: '综合推荐',
        list: [
  ...
  async function getList() {
    ...
    const { code, data } = await SpuApi.getSpuPage({
      ...
      isMilitary: state.isMilitary || undefined,
    });
    if (code !== 0) {
      return;
    }
  ...
  onLoad((options) => {
    state.categorySales = options.categorySales;
    state.isMilitary = options?.isMilitary === 'true' || options?.isMilitary === true;
