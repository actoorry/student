...
<script setup>
  ...
  // 非 TabBar 页面通过 navigateTo 跳转时，分类编号在页面 URL 参数中。
  let routeCategoryId;
  let redirectingToGoods = false;
  ...
  async function getGoodsList() {
    ...
    const res = await SpuApi.getSpuPage({
      categorySales: state.categoryList[state.activeMenu].id,
  ...
  function initMenuIndex() {
    ...
    // 兼容原生 TabBar 跳转参数；普通装修链接优先使用 onLoad 接收到的 URL 参数。
    const tabbarParams = appStore.paramsForTabbar || {};
    const id = routeCategoryId ?? tabbarParams.id;
  ...
  // 历史装修分类链接带 id 时直接进入商品列表；底部分类 Tab 无 id 时仍保留原分类页。
  onLoad((options) => {
    const categoryId = toPositiveCategoryId(options?.id);
    if (categoryId) {
      redirectingToGoods = true;
      sheep.$router.redirect('/pages/goods/list', { categorySales: categoryId });
      return;
    }
    routeCategoryId = options?.id;
  });
  onShow(async () => {
    if (redirectingToGoods) {
      return;
    }
    await getList();
    initMenuIndex();
