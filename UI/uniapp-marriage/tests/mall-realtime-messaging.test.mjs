...
test('Mall integrations keep one transport and preserve backend contracts', () => {
  const realtime = read('sheep/api/infra/realtime.js'); const privateApi = read('sheep/api/marriage/message.js'); const kefuApi = read('sheep/api/promotion/kefu.js'); const kefuPage = read('pages/chat/index.vue'); const privatePage = read('pages/messages/chat/index.vue'); const list = read('pages/chat/components/messageList.vue');
  ...
  assert.match(privateApi, /receiverId[\s\S]*limit[\s\S]*maxId/); assert.match(privateApi, /clientMessageId/);
  ...
  assert.match(privatePage, /reconcileLatest/); assert.match(privatePage, /startReconcileTimer/);
