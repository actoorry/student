...
const buildSpmQuery = (params) => {
  ...
  if (typeof params.shareId === 'undefined') {
    if (user.isLogin) {
      shareId = toPositiveInteger(user.userInfo.id);
    }
  } else {
    shareId = toPositiveInteger(params.shareId);
  }
...
const decryptSpm = async (spm) => {
  const user = $store('user');
  const shareParamsArray = typeof spm === 'string' ? spm.split('.') : [];
  let shareParams = {
    spm,
    shareId: 0,
    page: '',
  ...
  let query;
  shareParams.shareId = toPositiveInteger(shareParamsArray[0]);
  ...
  shareParams.platform = platformMap[Number(shareParamsArray[3]) - 1] || '';
  shareParams.from = fromMap[Number(shareParamsArray[4]) - 1] || '';
...
const bindBrokerageUser = async (val = undefined) => {
  try {
    const shareId = toPositiveInteger(val || uni.getStorageSync('shareId'));
    if (!shareId) {
      return false;
    }
    // 绑定成功返回 true，失败返回 false
    const result = await BrokerageApi.bindBrokerageUser({ bindUserId: shareId });
    // 绑定成功后清除缓存
    if (shouldClearPendingShareId(result)) {
