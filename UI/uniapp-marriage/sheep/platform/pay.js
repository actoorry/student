...
export default class SheepPay {
  constructor(payment, orderType, id, options = {}) {
  ...
  payAction() {
    ...
    const action = payAction[sheep.$platform.name]?.[this.payment];
    if (!action) {
      sheep.$helper.toast('当前环境不支持该支付方式');
      this.finish();
      return;
    }
    return action();
  ...
  async wechatWapPay() {
    ...
    if (error === 0) {
      const redirect_url = `${getRootUrl()}pages/accountant/result?id=${this.id}&payment=${
        this.payment
  ...
  async wechatVirtualPay() {
    const logFailure = (stage, error) => {
      console.error(
        '[wechatVirtualPay]',
        buildWechatVirtualPaymentErrorSummary(stage, error),
      );
    };

    let socialUser;
    try {
      socialUser = await sheep.$platform.useProvider('wechat')?.getInfo?.();
    } catch (error) {
      logFailure('identity', error);
      sheep.$helper.toast('微信身份读取失败，请重新登录后重试');
      this.finish();
      return;
    }
    const openid = socialUser?.openid;
    if (!openid) {
      logFailure('identity', { code: 'WECHAT_SOCIAL_USER_MISSING' });
      sheep.$helper.toast('请重新登录或绑定微信后再支付');
      this.finish();
      return;
    }
    ...
    let submitResult;
    try {
      submitResult = await WechatVirtualPayApi.submit(this.id, openid);
    } catch (error) {
      logFailure('submit', error);
      sheep.$helper.toast('虚拟支付请求未能提交，请检查网络后重试');
      this.finish();
      return;
    }
    const { code, data } = submitResult || {};
    if (code !== 0 || !data) {
      logFailure('submit-response', submitResult);
      sheep.$helper.toast('获取虚拟支付参数失败，请稍后重试');
      this.finish();
      return;
    }
    ...
    let paymentParams;
    try {
      paymentParams = buildRequestVirtualPaymentParams(data);
    } catch (error) {
      logFailure('parameters', error);
      sheep.$helper.toast('虚拟支付参数异常，请稍后重试');
      this.finish();
      return;
    }
    ...
    if (typeof wx === 'undefined' || typeof wx.requestVirtualPayment !== 'function') {
      logFailure('capability', { code: 'REQUEST_VIRTUAL_PAYMENT_UNAVAILABLE' });
      sheep.$helper.toast('当前微信版本不支持虚拟支付，请升级后重试');
      this.finish();
      return;
    }
    try {
      wx.requestVirtualPayment({
        ...
        success: () => {
          this.finish();
          this.payResult(undefined, true);
        },
        fail: (error) => {
          if (isWechatVirtualPaymentCancelled(error)) {
            sheep.$helper.toast('支付已取消，可继续支付');
          } else {
            logFailure('native', error);
            sheep.$helper.toast('虚拟支付失败，请稍后重试');
          }
          this.finish();
        },
      });
    } catch (error) {
      logFailure('native-invoke', error);
      sheep.$helper.toast('虚拟支付调用失败，请稍后重试');
      this.finish();
    }
    // #endif
    // #ifndef MP-WEIXIN
    logFailure('platform', { code: 'WECHAT_MINI_PROGRAM_REQUIRED' });
    sheep.$helper.toast('当前环境不支持虚拟支付');
    this.finish();
    // #endif
  }
  ...
  payResult(resultType, virtual = false) {
    goPayResult(this.id, this.orderType, resultType, virtual);
  }
...
// 支付结果跳转,success:成功，fail:失败
export function goPayResult(id, orderType, resultType, virtual = false) {
  sheep.$router.redirect('/pages/accountant/result', {
    id,
    orderType,
    payState: resultType,
