...
import SocialApi from '@/sheep/api/partner/social';
...
async function getLoginResult() {
  try {
    return (await uni.login()) || null;
  } catch (error) {
    console.warn('[wechat] uni.login failed', error);
    return null;
  }
}
...
const login = async () => {
  return new Promise(async (resolve, reject) => {
    // 1. 获得微信 code
    const codeResult = await getLoginResult();
    if (!codeResult || codeResult.errMsg !== 'login:ok' || !codeResult.code) {
...
const mobileLogin = async (e) => {
  return new Promise(async (resolve, reject) => {
    if (!e || e.errMsg !== 'getPhoneNumber:ok' || !e.code) {
      return resolve(false);
    }
    ...
    // 2. Use the current Partner Mini Program contract.
    const loginResult = await AuthUtil.weixinMiniLogin({
      code: codeResult.code,
      phoneCode: e.code,
    });
    if (loginResult.code === 0) {
      const socialUser = await getInfo();
      if (socialUser?.openid) setOpenid(socialUser.openid);
      return resolve(true);
    } else {
      return resolve(false);
    }
...
async function getSubscribeTemplate() {
  try {
    const response = await SocialApi.getSubscribeTemplateList();
    if (response?.code === 0 && Array.isArray(response.data)) {
      subscribeEventList = response.data;
    }
  } catch (error) {
    console.warn('[wechat] failed to load subscribe templates', error);
  }
}
