...
// 打开授权弹框
export function showAuthModal(type = 'wechatMiniLogin') {
...
export function showRealNameVerificationModal(scene) {
  $store('modal').$patch((state) => {
    state.realNameVerification = scene;
  });
}

export function closeRealNameVerificationModal() {
  $store('modal').$patch((state) => {
    state.realNameVerification = '';
  });
}
