...
http.interceptors.request.use(
  (config) => {
    // 自定义处理【auth 授权】：拒绝未登录请求；登录只允许由 Mine 的明确授权动作发起。
    ...
    // 增加 token 令牌、terminal 终端、tenant 租户和诊断性客户端上下文请求头
    ...
    const clientContext = getClientContext($platform.name);
    config.header['X-Client-Platform'] = clientContext.platform;
    config.header['X-App-Version'] = clientContext.appVersion;
    return config;
  },
  (error) => {
...
http.interceptors.response.use(
  (response) => {
    ...
    if (response.config.url.indexOf('/partner/auth/') >= 0 && response.data?.data?.accessToken) {
  ...
  (error) => {
    ...
    const errorDetail =
      typeof safeError.errMsg === 'string'
        ? safeError.errMsg
    ...
    if (error && typeof error === 'object') {
      ...
      if (errorDetail.toLowerCase().includes('timeout')) errorMessage = '请求超时';
      // #ifdef H5
    ...
    if (safeError.config) {
      const custom = safeError.config.custom || options;
      if (custom.showError) {
        uni.showToast({
          title: safeError.data?.msg || errorMessage,
          icon: 'none',
          mask: true,
        });
      }
...
const refreshToken = async (config) => {
  ...
  if (config.url.indexOf('/partner/auth/refresh-token') >= 0) {
    return Promise.reject('error');
  }
...
const handleAuthorized = () => {
  ...
  // 登录仅能由 Mine 的明确授权动作发起，不因 401 自动打开认证弹层。
