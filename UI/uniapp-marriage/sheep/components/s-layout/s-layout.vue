<template>
  <view
    class="page-app marriage-theme"
    :class="['theme-' + sys?.mode, 'main-' + sys?.theme, 'font-' + sys?.fontSize]"
  >
    <view class="page-main" :style="[bgMain, viewportLayout.main]">
      <!-- 顶部导航栏-情况1：默认通用顶部导航栏 -->
      <su-navbar
        v-if="navbar === 'normal'"
        :title="title"
        ...
        :color="color"
        :tools="tools"
        :leftIcon="shouldShowBackButton ? 'left' : ''"
      ...
      <s-custom-navbar
        ...
        :data="navbarStyle"
        :showLeftButton="shouldShowBackButton"
      />
      <view class="page-body" :style="[bgBody, viewportLayout.body]">
        <!-- 顶部导航栏-情况3：沉浸式头部 -->
        <su-inner-navbar
          v-if="navbar === 'inner'"
          :title="title"
    ...
    <view class="page-modal">
      ...
      <!-- 全局快捷入口 -->
      <s-menu-tools />
      <s-real-name-verification-modal />
    </view>
  </view>
</template>
...
<script setup>
  ...
  const pageShareInfo = ref({});
  provide('setPageShareInfo', (value) => {
    pageShareInfo.value = value || {};
  });

  // 底部主导航页不显示返回按钮，其余页面统一使用全局返回按钮。
  // 保留 showLeftButton 作为显式开启入口，兼容已有页面调用。
  const shouldShowBackButton = computed(() => props.showLeftButton || !Boolean(props.tabbar));
  const viewportLayout = computed(() => getViewportLayoutStyles(props.fitViewport));
  ...
  const shareInfo = computed(() => {
    if (props.onShareAppMessage === true) {
      return !isEmpty(pageShareInfo.value)
        ? pageShareInfo.value
...
<style lang="scss" scoped>
  .page-app {
    position: relative;
    color: var(--ui-TC);
    background-color: var(--marriage-soft, var(--ui-BG-1)) !important;
    z-index: 2;
    display: flex;
    width: 100%;
    height: 100vh;
