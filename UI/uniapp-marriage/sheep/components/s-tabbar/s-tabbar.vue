  ...
  <view class="u-page__item" v-if="tabbar?.items?.length > 0">
    ...
    >
      <su-tabbar-item
        ...
        @click="handleTabbarClick(item.url)"
      >
...
<script setup>
  ...
  import { shouldNavigateTabbar } from '@/sheep/helper/tabbar-navigation.mjs';
  ...
  const handleTabbarClick = (target) => {
    if (shouldNavigateTabbar(props.path, target)) {
      sheep.$router.go(target);
    }
  };
