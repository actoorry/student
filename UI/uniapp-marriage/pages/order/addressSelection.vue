  ...
  <view class="allAddress" :style="state.isPickUp ? '' : 'padding-top:10rpx;'">
    <view class="delivery-modes">
      <view
        ...
        :key="mode.type"
        class="delivery-mode"
        :class="{ active: state.deliveryType === mode.type }"
        @tap="switchDeliveryType(mode.type)"
      >
        <view class="delivery-mode-name">{{ mode.name }}</view>
        <view class="delivery-mode-desc">{{ mode.description }}</view>
      </view>
    </view>
    <view v-if="!availableModes.length" class="delivery-empty">当前商品没有共同配送方式</view>
    <!-- 情况一：收货地址的选择 -->
    <view
...
<script setup>
  ...
  const modeDefinitions = [
    { type: 0, name: '线上发货', description: '付款后由商家在线处理' },
    { type: 1, name: '快递发货', description: '需要填写收货地址' },
    { type: 2, name: '用户自提', description: '需要选择门店和联系人' },
    { type: 3, name: '自动发货', description: '付款后系统自动处理' },
  ];
  const availableModes = computed(() =>
    modeDefinitions.filter(
      (mode) => props.availableTypes.includes(mode.type) && (mode.type !== 2 || state.value.isPickUp),
    ),
  );
