...
<template>
  <s-layout title="我的团队" :class="state.scrollTop ? 'team-wrap' : ''" navbar="inner">
    ...
    <view v-else-if="state.error" class="page-state page-state--error" @tap="getTeamList">
      {{ state.error }}，点击重试
    </view>
...
<script setup>
  ...
  async function getTeamList() {
    ...
    if (state.sortKey) {
      params.sortAsc = state.sortAsc;
      params.sortField = state.sortKey;
    }
    if (state.nickname) {
      params.nickname = state.nickname;
    }
    try {
      const { code, data } = await BrokerageApi.getBrokerageUserChildSummaryPage(params);
      if (code !== 0) {
        state.error = '团队信息暂时无法读取';
        return;
      }
      ...
      state.error = error?.message || '团队信息暂时无法读取';
    } finally {
      state.loading = false;
    }
  }
