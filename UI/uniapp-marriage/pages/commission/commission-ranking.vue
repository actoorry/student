...
<template>
  <s-layout title="佣金排行">
    ...
    <view v-else-if="state.error" class="page-state page-state--error" @tap="getBrokerageRankList">
      {{ state.error }}，点击重试
    </view>
...
<script setup>
  ...
  async function getBrokerageRankList() {
    ...
    try {
      ...
      if (code !== 0) {
        state.error = '佣金排行暂时无法读取';
        return;
      }
      state.pagination.list = concat(state.pagination.list, data?.list || []);
      state.pagination.total = data?.total || 0;
      state.loadStatus = state.pagination.list.length < state.pagination.total ? 'more' : 'noMore';
      if (state.pagination.pageNo === 1) getBrokerageRankNumber();
    } catch (error) {
      state.error = error?.message || '佣金排行暂时无法读取';
    } finally {
      state.loading = false;
    }
  }
