    ...
    <view class="draw-card">
      <view class="bank-box ss-flex ss-col-center ss-row-between ss-m-b-30">
        ...
        <view class="bank-list ss-flex ss-col-center" @tap="onAccountSelect(true)">
          ...
          <view v-if="state.accountInfo.type === '6'" class="empty-text">支付宝余额</view>
      ...
      <view v-if="state.error" class="page-error">{{ state.error }}</view>
      <button
        class="ss-reset-button save-btn ui-BG-Main-Gradient ui-Shadow-Main"
        :disabled="state.submitting || state.loading || !state.withdrawTypes.length"
        @tap="onConfirm"
      >
        {{ state.submitting ? '提交中…' : '确认提现' }}
      </button>
    </view>
...
<script setup>
  ...
  import {
    normalizeWithdrawTypes,
    validateBrokerageWithdraw,
  ...
  const onConfirm = async () => {
    ...
    const validation = validateBrokerageWithdraw({
      priceYuan: state.accountInfo.price,
      withdrawablePrice: state.brokerageInfo.brokeragePrice,
      minPrice: state.minPrice,
      type: state.accountInfo.type,
      withdrawTypes: state.withdrawTypes,
    ...
    try {
      ...
      const data = {
        ...state.accountInfo,
        price: Math.round(Number(state.accountInfo.price) * 100),
      };
      if (state.accountInfo.type === '5') {
      ...
      const result = await BrokerageApi.createBrokerageWithdraw(data);
      if (result?.code !== 0) {
        sheep.$helper.toast(result?.msg || '提现申请提交失败，请稍后重试');
        return;
      }
      resetAccountInfo();
      await getBrokerageUser();
      uni.showModal({
        title: '操作成功',
        content: '您的提现申请已成功提交',
        cancelText: '继续提现',
        confirmText: '查看记录',
        success: (res) => {
          if (res.confirm) sheep.$router.go('/pages/commission/wallet', { type: 2 });
        },
      });
    } catch (error) {
      sheep.$helper.toast(error?.message || '提现申请提交失败，请稍后重试');
    } finally {
  ...
  async function getWithdrawRules() {
    try {
      const { code, data } = await SalesConfigApi.getSalesConfig();
      if (code !== 0 || !data) {
        state.error = '提现配置暂时无法读取，请稍后重试';
        return null;
      }
      state.minPrice = data.brokerageWithdrawMinPrice || 0;
      state.frozenDays = data.brokerageFrozenDays || 0;
      state.withdrawTypes = normalizeWithdrawTypes(data.brokerageWithdrawTypes);
      if (!state.withdrawTypes.length) state.error = '当前暂无可用的提现方式';
      return data;
    } catch (error) {
      state.error = error?.message || '提现配置暂时无法读取，请稍后重试';
      return null;
    }
  }
  ...
  async function getBrokerageUser() {
    try {
      ...
      state.error = '分销账户暂时无法读取，请稍后重试';
    } catch (error) {
      state.error = error?.message || '分销账户暂时无法读取，请稍后重试';
    }
    return null;
  }
  ...
  async function getDictDataListByType() {
    try {
      const { code, data } = await DictApi.getDictDataListByType('brokerage_bank_name');
      if (code === 0 && data?.length) state.bankList = data;
    } catch (error) {
      // 只有银行卡方式依赖该字典，其他提现方式仍可正常使用。
      console.warn('brokerage bank dictionary unavailable', error);
    }
  }
  ...
  async function loadPage() {
    state.loading = true;
    state.error = '';
    await Promise.allSettled([getWithdrawRules(), getBrokerageUser(), getDictDataListByType()]);
    state.loading = false;
  }
