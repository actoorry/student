...
<template>
  <s-layout title="分销订单" :class="state.scrollTop ? 'order-warp' : ''" navbar="inner">
    ...
    <view v-else-if="state.error" class="page-state page-state--error" @tap="getOrderList">
      {{ state.error }}，点击重试
    </view>
...
<script setup>
  ...
  async function getOrderList() {
    ...
    try {
      ...
      if (code !== 0) {
        state.error = '分销订单暂时无法读取';
        return;
      }
      state.pagination.list = concat(state.pagination.list, data?.list || []);
      state.pagination.total = data?.total || 0;
      state.loadStatus = state.pagination.list.length < state.pagination.total ? 'more' : 'noMore';
      if (state.currentTab === 0) state.totals = data?.total || 0;
    } catch (error) {
      state.error = error?.message || '分销订单暂时无法读取';
    } finally {
      state.loading = false;
    }
  }
