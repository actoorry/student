...
export const useUpload = (directory?: string, bizType?: string, bizId?: number) => {
  // 后端上传地址
  ...
  const httpRequest = async (options: UploadRequestOptions) => {
    ...
    if (isClientUpload) {
      ...
      return axios
        ...
        .then(() => {
          // 1.4. 记录文件信息到后端（异步）
          createFile(presignedInfo, options.file, fileName, bizType, bizId)
      ...
      return new Promise((resolve, reject) => {
        FileApi.updateFile({ file: options.file, directory, bizType, bizId }, uploadProgressHandler)
          .then((res) => {
            if (res.code === 0) {
              resolve(res)
            } else {
