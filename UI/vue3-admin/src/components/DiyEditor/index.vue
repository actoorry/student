  ...
  <el-container class="editor">
    ...
    <el-header class="editor-header">
      ...
      <el-button-group class="header-right">
        ...
        <el-tooltip v-if="showReset" content="重置">
        ...
        <el-tooltip v-if="showSave" content="保存">
    ...
    <el-container class="editor-container">
              ...
              <ComponentContainer
                ...
                :class="{
                  'sticky-component-container':
                    element.layout === 'sticky' && element.property?.sticky !== false
                }"
      ...
      <el-aside v-if="selectedComponent?.property" class="editor-right" width="350px">
        ...
        >
          ...
          >
            <component
              :is="selectedComponent?.propertyPanel || selectedComponent?.id + 'Property'"
...
import RetiredDiyComponent from './components/RetiredDiyComponent.vue'
...
import { DiyComponent, DiyComponentLibrary, DiyPageContext, PageConfig } from '@/components/DiyEditor/util'
import { componentConfigs } from '@/components/DiyEditor/components/mobile'
import { array, oneOfType } from 'vue-types'
import { propTypes } from '@/utils/propTypes'

// Persisted component IDs that no longer have an implementation. They are
// represented only while loading historical page JSON and never appear in a
// component library.
const RETIRED_EDITOR_COMPONENT_IDS = new Set([
  'MarriageLoginPrompt',
...
const createRetiredDiyComponent = (item: {
  id: string
  property: DiyComponent<any>['property']
}): DiyComponent<any> | undefined => {
  if (!RETIRED_EDITOR_COMPONENT_IDS.has(item.id)) {
    return undefined
  }
  return {
    id: item.id,
    ...
    preview: markRaw(RetiredDiyComponent),
    propertyPanel: markRaw(RetiredDiyComponent)
  }
}
...
const props = defineProps({
  ...
  // 业务包装层注入的组件定义（预览与属性面板均可自定义）。
  componentDefinitions: propTypes.object.def({}),
  // 页面上下文：home 首页 / user 我的 / standalone 独立页面
  pageContext: propTypes.string.def('standalone'),
  // 是否显示顶部导航栏
  ...
  // 是否显示核心重置操作；业务包装层可自行提供带权限控制的操作按钮。
  showReset: propTypes.bool.def(true),
  // 是否显示核心保存操作；业务包装层可自行提供带权限控制的保存按钮。
  ...
  () => {
    const modelValue =
      isString(props.modelValue) && !isEmpty(props.modelValue)
        ...
        : props.modelValue
    // 对象模式下，内部修改会通过 v-model 以新对象回传。若内容与当前编辑状态一致，
    // 不再重复重建 pageComponents，避免“内部更新 -> 父级回传 -> 内部更新”的递归循环。
    if (modelValue && isEqual(modelValue, getCurrentPageConfig())) {
      return
    }
    ...
    // 查找对应的页面组件
    pageComponents.value = ((typeof modelValue !== 'string' && modelValue?.components) || [])
      .map((item) => {
        const component =
          props.componentDefinitions[item.id] ||
          componentConfigs[item.id] ||
          createRetiredDiyComponent(item)
        return component ? { ...component, property: item.property } : null
      })
      .filter((item): item is DiyComponent<any> => item !== null)
  },
  {
    immediate: true
  }
...
const pageConfigChange = () => {
  ...
  // 发送数据更新通知
  // 后端持久化契约是 JSON 字符串；空值首次编辑时也必须切换到字符串模式。
  const modelValue =
    isString(props.modelValue) || props.modelValue == null ? JSON.stringify(pageConfig) : pageConfig
