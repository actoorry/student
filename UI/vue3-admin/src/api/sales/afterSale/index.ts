...
export const getAfterSalePage = async (params) => {
  return await request.get({ url: `/sales/after-sale/page`, params })
}

// 获得交易售后详情
export const getAfterSale = async (id: any) => {
  return await request.get({ url: `/sales/after-sale/get-detail?id=${id}` })
}

// 同意售后
export const agree = async (id: any) => {
  return await request.put({ url: `/sales/after-sale/agree?id=${id}` })
}

// 拒绝售后
export const disagree = async (data: any) => {
  return await request.put({ url: `/sales/after-sale/disagree`, data })
}

// 确认收货
export const receive = async (id: any) => {
  return await request.put({ url: `/sales/after-sale/receive?id=${id}` })
}

// 拒绝收货
export const refuse = async (id: any) => {
  return await request.put({ url: `/sales/after-sale/refuse?id=${id}` })
}

// 确认退款
export const refund = async (id: any) => {
  return await request.put({ url: `/sales/after-sale/refund?id=${id}` })
}
