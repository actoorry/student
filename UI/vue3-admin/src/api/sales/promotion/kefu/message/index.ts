...
export const KeFuMessageApi = {
  ...
  updateKeFuMessageReadStatus: async (conversationId: number) => {
    return await request.put({
      url: '/sales/promotion/kefu-message/update-read-status?conversationId=' + conversationId
    })
  },
  // 获得消息列表（流式加载）
  getKeFuMessageList: async (params: any) => {
    return await request.get({ url: '/sales/promotion/kefu-message/list', params })
  }
}
