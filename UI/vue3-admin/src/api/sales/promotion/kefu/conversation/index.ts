...
export const KeFuConversationApi = {
  ...
  getConversationList: async () => {
    return await request.get({ url: '/sales/promotion/kefu-conversation/list' })
  },
  // 获得客服会话
  getConversation: async (id: number) => {
    return await request.get({ url: `/sales/promotion/kefu-conversation/get?id=` + id })
  },
  // 客服会话置顶
  updateConversationPinned: async (data: any) => {
    return await request.put({
      url: '/sales/promotion/kefu-conversation/update-conversation-pinned',
      data
    })
  },
  // 删除客服会话
  deleteConversation: async (id: number) => {
    return await request.delete({ url: `/sales/promotion/kefu-conversation/delete?id=${id}` })
  }
}
