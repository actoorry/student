...
import { Sku, Spu } from '@/api/product/spu' // 积分商城活动 VO
...
export const PointActivityApi = {
  ...
  getPointActivityPage: async (params: any) => {
    return await request.get({ url: `/sales/promotion/point-activity/page`, params })
  },

  // 查询积分商城活动详情
  getPointActivity: async (id: number) => {
    return await request.get({ url: `/sales/promotion/point-activity/get?id=` + id })
  },

  // 查询积分商城活动列表，基于活动编号数组
  getPointActivityListByIds: async (ids: number[]) => {
    return request.get({ url: `/sales/promotion/point-activity/list-by-ids?ids=${ids}` })
  },

  // 新增积分商城活动
  createPointActivity: async (data: PointActivityVO) => {
    return await request.post({ url: `/sales/promotion/point-activity/create`, data })
  },

  // 修改积分商城活动
  updatePointActivity: async (data: PointActivityVO) => {
    return await request.put({ url: `/sales/promotion/point-activity/update`, data })
  },

  // 删除积分商城活动
  deletePointActivity: async (id: number) => {
    return await request.delete({ url: `/sales/promotion/point-activity/delete?id=` + id })
  },

  // 关闭秒杀活动
  closePointActivity: async (id: number) => {
    return await request.put({ url: '/sales/promotion/point-activity/close?id=' + id })
  }
}
