...
export const getDeliveryExpressTemplatePage = async (params: PageParam) => {
  return await request.get({ url: '/sales/delivery/express-template/page', params })
}

// 查询快递运费模板详情
export const getDeliveryExpressTemplate = async (id: number) => {
  return await request.get({ url: '/sales/delivery/express-template/get?id=' + id })
}

// 查询快递运费模板详情
export const getSimpleTemplateList = async () => {
  return await request.get({ url: '/sales/delivery/express-template/list-all-simple' })
}

// 新增快递运费模板
export const createDeliveryExpressTemplate = async (data: DeliveryExpressTemplateVO) => {
  return await request.post({ url: '/sales/delivery/express-template/create', data })
}

// 修改快递运费模板
export const updateDeliveryExpressTemplate = async (data: DeliveryExpressTemplateVO) => {
  return await request.put({ url: '/sales/delivery/express-template/update', data })
}

// 删除快递运费模板
export const deleteDeliveryExpressTemplate = async (id: number) => {
  return await request.delete({ url: '/sales/delivery/express-template/delete?id=' + id })
}
