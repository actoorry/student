...
export interface OrderVO {
  ...
  // 电子面单信息
  waybill?: ElectronicWaybillVO // 电子面单信息（无有效面单时为空）
  // 下单用户信息
  user?: {
    id?: number | null
    nickname?: string
...
export const getOrderPage = async (params: any) => {
  return await request.get({ url: `/sales/order/page`, params })
}

// 查询交易订单统计
export const getOrderSummary = async (params: any) => {
  return await request.get<TradeOrderSummaryRespVO>({ url: `/sales/order/summary`, params })
}

// 查询交易订单详情
export const getOrder = async (id: number | null) => {
  return await request.get({ url: `/sales/order/get-detail?id=` + id })
}

// 查询交易订单物流详情
export const getExpressTrackList = async (id: number | null) => {
  return await request.get({ url: `/sales/order/get-express-track-list?id=` + id })
}
...
export const deliveryOrder = async (data: DeliveryVO) => {
  return await request.put({ url: `/sales/order/delivery`, data })
}

/** 电子面单信息 */
export interface ElectronicWaybillVO {
  id?: number // 电子面单记录编号
  status?: number // 状态：0-失败 1-有效 2-已作废
  ...
  labelUrl?: string // 面单短链（可打印内容）
  failCode?: string // 失败错误码
  failMessage?: string // 失败摘要
...
/** 电子面单发货请求 */
export interface WaybillDeliveryVO {
  id?: number // 订单编号
  accountId?: number // 电子面单账户编号
  addressId?: number // 寄件地址编号（为空使用账户默认）
  printType?: string // 打印类型：HTML / IMAGE
}

/** 电子面单取消请求 */
export interface WaybillCancelVO {
  id?: number // 订单编号
  reason?: string // 取消原因
}

// 订单电子面单发货
export const deliveryOrderByWaybill = async (data: WaybillDeliveryVO) => {
  return await request.put({ url: `/sales/order/delivery-by-waybill`, data })
}

// 电子面单复打
export const reprintElectronicWaybill = async (id: number) => {
  return await request.put<ElectronicWaybillVO>({ url: `/sales/order/waybill-reprint?id=${id}` })
}

// 电子面单取消
export const cancelElectronicWaybill = async (data: WaybillCancelVO) => {
  return await request.put({ url: `/sales/order/waybill-cancel`, data })
}

// 订单备注
export const updateOrderRemark = async (data: any) => {
  return await request.put({ url: `/sales/order/update-remark`, data })
}

// 订单调价
export const updateOrderPrice = async (data: any) => {
  return await request.put({ url: `/sales/order/update-price`, data })
}

// 修改订单地址
export const updateOrderAddress = async (data: any) => {
  return await request.put({ url: `/sales/order/update-address`, data })
}

// 订单核销
export const pickUpOrder = async (id: number) => {
  return await request.put({ url: `/sales/order/pick-up-by-id?id=${id}` })
}

// 订单核销
export const pickUpOrderByVerifyCode = async (pickUpVerifyCode: string) => {
  return await request.put({
    url: `/sales/order/pick-up-by-verify-code`,
...
export const getOrderByPickUpVerifyCode = async (pickUpVerifyCode: string) => {
  return await request.get<OrderVO>({
    url: `/sales/order/get-by-pick-up-verify-code`,
    params: { pickUpVerifyCode }
  })
}
