...
export enum DICT_TYPE {
  ...
  PRODUCT_UNIT_TYPE = 'product_unit_type', // 产品单位类型
  ...
  MES_WM_ARRIVAL_NOTICE_STATUS = 'wms_arrival_notice_status', // MES 到货通知单状态
  MES_WM_ITEM_RECEIPT_STATUS = 'wms_item_receipt_status', // MES 物料接收单状态
  MES_WM_TRANSFER_STATUS = 'wms_transfer_status', // MES 转移单状态
  MES_WM_TRANSFER_TYPE = 'wms_transfer_type', // MES 转移单类型
  MES_WM_STOCK_TAKING_TYPE = 'wms_stock_taking_type', // MES 盘点类型
  MES_WM_STOCK_TAKING_TASK_STATUS = 'wms_stock_taking_task_status', // MES 盘点任务状态
  MES_WM_STOCK_TAKING_LINE_STATUS = 'wms_stock_taking_task_line_status', // MES 盘点任务行状态
  MES_WM_STOCK_TAKING_PLAN_PARAM_TYPE = 'wms_stock_taking_plan_param_type', // MES 盘点方案参数类型
  MES_WM_OUTSOURCE_RECPT_STATUS = 'wms_outsource_recpt_status', // MES 外协入库单状态
  MES_WM_PRODUCT_ISSUE_STATUS = 'wms_product_issue_status', // MES 领料出库单状态
  MES_WM_PRODUCT_PRODUCE_STATUS = 'wms_product_produce_status', // MES 生产入库单状态
  MES_WM_RETURN_VENDOR_STATUS = 'wms_return_vendor_status', // MES 供应商退货单状态
  MES_WM_QUALITY_STATUS = 'wms_quality_status', // MES 质量状态
  MES_WM_RETURN_ISSUE_STATUS = 'wms_return_issue_status', // MES 生产退料单状态
  MES_WM_RETURN_ISSUE_TYPE = 'wms_return_issue_type', // MES 退料类型
  MES_WM_PRODUCT_RECPT_STATUS = 'wms_product_receipt_status', // MES 成品入库单状态
  MES_WM_RETURN_SALES_STATUS = 'wms_return_sales_status', // MES 销售退货单状态
  MES_WM_PRODUCT_SALES_STATUS = 'wms_product_sales_status', // MES 销售出库单状态
  MES_WM_SALES_NOTICE_STATUS = 'wms_sales_notice_status', // MES 发货通知单状态
  MES_WM_MISC_ISSUE_TYPE = 'wms_misc_issue_type', // MES 杂项出库类型
  MES_WM_MISC_ISSUE_STATUS = 'wms_misc_issue_status', // MES 杂项出库单状态
  MES_WM_MISC_RECEIPT_TYPE = 'wms_misc_receipt_type', // MES 杂项单类型
  MES_WM_MISC_RECEIPT_STATUS = 'wms_misc_receipt_status', // MES 杂项入库单状态
  MES_WM_OUTSOURCE_RECEIPT_STATUS = 'wms_outsource_receipt_status', // MES 外协入库单状态
  MES_WM_OUTSOURCE_ISSUE_STATUS = 'wms_outsource_issue_status', // MES 外协出库单状态
  ...
  MES_WM_BARCODE_FORMAT = 'wms_barcode_format', // MES 条码格式
  MES_WM_BARCODE_BIZ_TYPE = 'wms_barcode_biz_type', // MES 条码业务类型
  MES_WM_PACKAGE_STATUS = 'wms_package_status', // MES 装箱单状态

  // ========== HR 人事模块 ==========
  HR_PERSONNEL_CATEGORY = 'hr_personnel_category', // 工作类别（OA work_class）
  HR_PEOPLE_TYPE = 'hr_people_type', // 人员类别（OA people_type）
  HR_PERSONNEL_IDENTITY = 'hr_personnel_identity', // 人员身份
  HR_RECRUITMENT_SOURCE = 'hr_recruitment_source', // 人员来源
  HR_POLITICAL_STATUS = 'hr_political_status', // 政治面貌
  HR_ETHNICITY = 'hr_ethnicity', // 民族
  HR_EDUCATION = 'hr_education', // 学历
  HR_PROFESSIONAL_TITLE = 'hr_professional_title', // 技术职称
  HR_PROFESSIONAL_CATEGORY = 'hr_professional_category', // 职称分类（图表辅助，员工页不用）
  HR_DUTY = 'hr_duty', // 职务（OA 为文本输入，员工页不用）
  HR_POSITION = 'hr_position', // 岗位（已废弃，保留常量避免引用报错）
  HR_EMPLOYMENT_STATUS = 'hr_employment_status', // 在职状态
  HR_ENTRY_MODE = 'hr_entry_mode', // 入编方式
  HR_ESTABLISHMENT_STATUS = 'hr_establishment_status', // 编制状态
  HR_WORKING_TYPE = 'hr_working_type', // 工作类型（OA workingType，合同模块用）

  // ========== HR 证书督查 ==========
  HR_CERTIFICATE_TYPE = 'hr_certificate_type', // 证书类型（医师资格证/执业证/护士证等）

  // ========== HR 外出管理 ==========
  HR_OUTBOUND_TYPE = 'hr_outbound_type', // 外出类型（下乡支援/进修/学习/培训）
  HR_POST_CATEGORY = 'hr_post_category', // 岗位类别
  HR_POST_LEVEL = 'hr_post_level', // 岗位等级
  HR_APPOINTMENT_STATUS = 'hr_appointment_status', // 聘任状态
  HR_POSITION_CHANGE_TYPE = 'hr_position_change_type', // 岗位异动类型
  ...
  WMS_STOCK_MODE = 'wms_stock_mode', // 库存管控方式
  WMS_CHECK_TYPE = 'wms_check_type', // 盘点类型
  WMS_ADJUST_STATUS = 'wms_adjust_status', // 盘点调整状态
  WMS_BIZ_TYPE = 'wms_biz_type', // 库存流水业务类型
  WMS_REMOVAL_STRATEGY = 'wms_removal_strategy', // 出库策略
}
