<!--
  ...
  对齐 PartnerSelectDialog 架构模式 + userSelect 左侧部门树
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="5" :xs="24">
        <ContentWrap class="h-full">
          <DeptTreeSelect
            ref="deptTreeRef"
            tree-max-height="calc(60vh - 80px)"
...
const emit = defineEmits<{
  selected: [rows: UserApi.PartnerVO[]]
}>()

const dialogVisible = ref(false) // 弹窗是否展示
const loading = ref(false) // 列表加载中
const list = ref<UserApi.PartnerVO[]>([]) // 用户列表
...
const selectedRows = ref<UserApi.PartnerVO[]>([]) // 多选模式：选中行
const selectedRadioId = ref<number>() // 单选模式：选中 ID
const currentRadioRow = ref<UserApi.PartnerVO>() // 单选模式：选中行对象
const preSelectedIds = ref<number[]>([]) // 打开弹窗时传入的已选 ID

/** 多选：checkbox 变化 */
const handleSelectionChange = (rows: UserApi.PartnerVO[]) => {
  if (props.multiple) {
...
/** 单选：radio 变化 */
const handleRadioChange = (row: UserApi.PartnerVO) => {
  currentRadioRow.value = row
}

/** 单击行：单选模式下点击整行即选中（降低操作成本），多选不处理（避免和 dblclick 冲突） */
const handleRowClick = (row: UserApi.PartnerVO) => {
  if (props.multiple) {
    return
  }
...
const handleRowDblClick = (row: UserApi.PartnerVO) => {
  if (props.multiple) {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await UserApi.getPartnerPage(queryParams)
