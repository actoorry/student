...
<template>
  ...
  >
    ...
    <el-form-item v-if="formData.specType" label="产品属性">
      <el-button class="mb-10px mr-15px" @click="openPropertyAddForm">添加属性</el-button>
      <ProductAttributes
        :is-detail="isDetail"
        :property-list="propertyList"
...
import {
  ...
  RuleConfig,
  SkuList
} from '@/views/product/spu/components/index'
...
const ruleConfig: RuleConfig[] = [
  {
    name: 'stock',
    rule: (arg) => arg >= 0,
    message: '产品库存必须大于等于 1 ！！！'
  },
  {
    name: 'price',
    rule: (arg) => arg >= 0.01,
    message: '产品销售价格必须大于等于 0.01 元！！！'
  },
  {
    name: 'marketPrice',
    rule: (arg) => arg >= 0.01,
    message: '产品市场价格必须大于等于 0.01 元！！！'
  },
  {
    name: 'costPrice',
    rule: (arg) => arg >= 0.01,
...
const attributesAddFormRef = ref() // 添加产品属性表单
const formRef = ref() // 表单 Ref
const propertyList = ref<PropertyAndValues[]>([]) // 产品属性列表
const skuListRef = ref() // 产品属性列表 Ref
...
const validate = async () => {
  if (!formRef) return
  try {
    formData.unitId = props.propFormData.unitId
    formData.type = props.propFormData.type
    // 校验 sku
    skuListRef.value.validateSku()
    await unref(formRef).validate()
    // 只回写库存价格页负责的字段，避免用本页缓存的 unitId 覆盖基础设置页的最新值
...
/** 打开添加属性弹窗 */
const openPropertyAddForm = async () => {
  await nextTick()
  if (!attributesAddFormRef.value?.open) {
    message.error('产品属性弹窗尚未准备好，请稍后重试')
    return
  }
