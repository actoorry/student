<template>
  ...
  >
    ...
    <el-table-column align="center" label="产品条码" min-width="168">
      <template #default="{ row }">
    ...
    <el-table-column v-if="showQuantityColumn" align="center" :label="quantityLabel" min-width="168">
      <template #default="{ row }">
        <el-input-number
  ...
  >
    ...
    <el-table-column align="center" label="产品条码" min-width="100">
      <template #default="{ row }">
        {{ row.barCode }}
      </template>
    </el-table-column>
    ...
    <el-table-column v-if="showQuantityColumn" align="center" :label="quantityLabel" min-width="80">
      <template #default="{ row }">
        {{ row.quantity }}
      </template>
    </el-table-column>
    <el-table-column
      v-if="isWechatVirtualGoodsProduct"
      align="center"
      label="微信道具"
      min-width="150"
    >
      <template #default="{ row }">
        <el-tag :type="formatWechatVirtualStatus(row).type">
          {{ formatWechatVirtualStatus(row).name }}
        </el-tag>
        <div v-if="row.wechatVirtualProductId" class="wechat-virtual-product-id">
          {{ row.wechatVirtualProductId }}
        </div>
        <div
          v-if="isWechatVirtualFailed(row) && row.wechatVirtualReviewFailReason"
          class="wechat-virtual-fail-reason"
        >
          微信审核未通过：{{ row.wechatVirtualReviewFailReason }}
        </div>
      </template>
    </el-table-column>
    <template v-if="formData!.subCommissionType">
...
import type { Property, Sku, Spu } from '@/api/product/spu'
import * as ProductUnitApi from '@/api/product/unit'
import { createImageViewer } from '@/components/ImageViewer'
import { RuleConfig } from '@/views/product/spu/components/index'
import { PropertyAndValues } from './index'
import { ElTable } from 'element-plus'
import { isEmpty } from '@/utils/is'
import { ProductTypeEnum, ProductWechatVirtualGoodsStatusEnum } from '@/utils/constants'
...
const unitList = ref<{ id: number; name: string }[]>([])
const isFulfillmentProduct = computed(() =>
  [
    ProductTypeEnum.SERVICE.type,
    ProductTypeEnum.MEMBER.type
  ].includes(formData.value?.type as number)
)
const isWechatVirtualGoodsProduct = computed(() =>
  formData.value?.isWechatMiniappVirtualGoods === true
)
const showQuantityColumn = computed(() => isFulfillmentProduct.value || !!formData.value?.unitId)
const quantityLabel = computed(() => {
  const unit = unitList.value.find((item) => item.id === formData.value?.unitId)
  return unit?.name ? `主单位数量(${unit.name})` : '主单位数量'
})
...
const validateSku = () => {
  validateProperty()
  let warningInfo = '请检查产品各行相关属性配置，'
  let validate = true // 默认通过
  for (const sku of formData.value!.skus!) {
    if (isFulfillmentProduct.value && (!sku.quantity || Number(sku.quantity) <= 0)) {
      message.warning('虚拟/会员/微信小程序虚拟商品数量必须大于 0')
      throw new Error('虚拟/会员/微信小程序虚拟商品数量必须大于 0')
    }
    // 作为活动组件的校验
...
const formatWechatVirtualStatus = (row: Sku) => {
  const status =
    row.wechatVirtualPublishStatus ??
    row.wechatVirtualReviewStatus ??
    row.wechatVirtualUploadStatus ??
    ProductWechatVirtualGoodsStatusEnum.NONE.status
  return (
    Object.values(ProductWechatVirtualGoodsStatusEnum).find((item) => item.status === status) ||
    ProductWechatVirtualGoodsStatusEnum.NONE
  )
}
const isWechatVirtualFailed = (row: Sku) =>
  [
    row.wechatVirtualUploadStatus,
    row.wechatVirtualPublishStatus,
    row.wechatVirtualReviewStatus
...
.wechat-virtual-product-id,
.wechat-virtual-fail-reason {
  margin-top: 4px;
  font-size: 12px;
  line-height: 16px;
}
