<template>
  <Dialog v-model="dialogVisible" :appendToBody="true" title="选择产品" width="70%">
    <ContentWrap>
      <el-form
        ref="queryFormRef"
        :inline="true"
      ...
      <el-table v-loading="loading" :data="list" show-overflow-tooltip>
        ...
        <el-table-column key="id" align="center" label="产品编号" prop="id" min-width="60" />
        <el-table-column label="产品图" min-width="80">
          <template #default="{ row }">
            <el-image
              :src="row.picUrl"
              class="h-30px w-30px"
        ...
        <el-table-column label="产品名称" min-width="200" prop="name" />
        <el-table-column label="产品分类" min-width="100" prop="categorySales">
          <template #default="{ row }">
            <span>{{ categoryList?.find((c) => c.id === row.categorySales)?.name }}</span>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
import { SALES_CATEGORY_ROOT_ID } from '../../category/categoryDimension'
 ...
 *    1.2 再次打开时，保持选中状态
 * 2. 多选模式：
 *    2.1 点击表格左侧的多选框时，记录选中的产品
 *    2.2 切换分页时，保持产品的选中的状态
