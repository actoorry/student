      ...
      <el-table-column label="操作" align="center" min-width="180">
        <template #default="scope">
          <el-button
            link
            type="primary"
            v-if="!isVirtualCategoryRoot(scope.row)"
          ...
          <el-button
            link
            type="primary"
            v-if="!isVirtualCategoryRoot(scope.row) && scope.row.parentId > 0"
...
import { isVirtualCategoryRoot, loadManagementCategoryTree } from './categoryDimension'
...
const getList = async () => {
  loading.value = true
  try {
    list.value = await loadManagementCategoryTree(queryParams.name)
  } finally {
    loading.value = false
  }
}
