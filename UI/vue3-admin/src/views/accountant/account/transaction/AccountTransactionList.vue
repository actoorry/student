<template>
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :show-overflow-tooltip="true" :stripe="true">
      ...
      <el-table-column align="center" label="账户编号" prop="walletId" />
...
import * as WalletTransactionApi from '@/api/accountant/account/transaction'
import * as WalletApi from '@/api/accountant/account'
import { fenToYuan } from '@/utils'

defineOptions({ name: 'AccountTransactionList' })
...
const getList = async () => {
  ...
  try {
    if (props.userId) {
      const wallet = await WalletApi.getAccount({ userId: props.userId })
      queryParams.walletId = wallet.id as any
    } else {
      queryParams.walletId = props.walletId as any
    }
    const data = await WalletTransactionApi.getAccountTransactionPage(queryParams)
