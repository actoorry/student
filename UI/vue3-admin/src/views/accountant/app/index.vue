<template>
  <doc-alert title="支付功能开启" url="https://doc.appap.vip/pay/build/" />
  <!-- 搜索 -->
  <ContentWrap>
    <el-form
      ref="queryFormRef"
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="账户支付配置">
  ...
  <WechatVirtualChannelForm ref="wechatVirtualFormRef" @success="getList" />
  <MockChannelForm ref="mockFormRef" @success="getList" />
  <WalletChannelForm ref="AccountFormRef" @success="getList" />
</template>
<script lang="ts" setup>
import { DICT_TYPE, getIntDictOptions } from '@/utils/dict'
import * as AppApi from '@/api/accountant/app'
...
import WechatVirtualChannelForm from './components/channel/WechatVirtualChannelForm.vue'
...
const wxChannels = [
  ...
  PayChannelEnum.WX_VIRTUAL_LITE,
...
const WX_VIRTUAL_LITE_CODE = 'wx_virtual_lite'
...
const wechatVirtualFormRef = ref()
...
const openChannelForm = async (row, payCode) => {
  ...
  if (payCode === WX_VIRTUAL_LITE_CODE) {
    wechatVirtualFormRef.value.open(row.id, payCode)
    return
  }
  ...
  if (payCode.indexOf('wallet') === 0) {
    AccountFormRef.value.open(row.id, payCode)
  }
}
