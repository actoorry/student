  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="70%">
    ...
    >
      <el-row>
        ...
        <el-col v-if="isWarriorCategory" :span="12">
          <el-form-item label="所属省份" prop="stateId">
            <el-select v-model="formData.stateId" clearable placeholder="请选择省份（留空=全国）">
              <el-option
        ...
        <template v-if="isHelpCategory">
          <el-col :span="12">
            <el-form-item label="帮扶金额" prop="helpAmount">
              <el-input-number
                v-model="formData.helpAmount"
                :min="0"
                :precision="2"
                :step="100"
          ...
          <el-col :span="12">
            <el-form-item label="申请人" prop="applicantName">
              <el-input v-model="formData.applicantName" placeholder="脱敏姓名，如 李**" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="执行状态" prop="executionStatus">
              <el-select v-model="formData.executionStatus" clearable placeholder="请选择执行状态">
...
import * as ArticleApi from '@/api/sales/promotion/article'
import * as ArticleCategoryApi from '@/api/sales/promotion/articleCategory'
import * as ProductSpuApi from '@/api/product/spu'
import { getAreaTree } from '@/api/system/area'
import { SpuSelect } from '@/views/sales/promotion/components'
...
const isWarriorCategory = computed(() => Number(formData.value.categoryId) === 4)
const isHelpCategory = computed(() => Number(formData.value.categoryId) === 5)
...
const resetForm = () => {
  ...
  if (route.meta.query) {
    formData.value.categoryId = Number(route.meta.query['categoryId']);
  }
...
onMounted(async () => {
  ...
  if (route.meta.query) {
    // Object.assign(formData.value, route.meta.query)
    // formData.value.categoryId = route.query.meta.categoryId;
  }
  categoryList.value =
  ...
  const areaTree = (await getAreaTree()) as Array<any>
  provinceList.value = (areaTree || []).map((item) => ({ id: item.id, name: item.name }))
  // formData.value.categoryId = route.meta.query.categoryId;// 如果 categoryId 是通过 URL 传递的查询参数
