...
import * as DiyTemplateApi from '@/api/sales/promotion/diy/template'
import * as DiyPageApi from '@/api/sales/promotion/diy/page'
import { useTagsViewStore } from '@/store/modules/tagsView'
import { DiyComponentLibrary, DiyPageContext, PAGE_LIBS } from '@/components/DiyEditor/util' // 商城的 DIY 组件，在 DiyEditor 目录下
...
const currentPageContext = computed<DiyPageContext>(() => {
  if (selectedTemplateItem.value === 1) return 'home'
  if (selectedTemplateItem.value === 2) return 'user'
  return 'standalone'
})
