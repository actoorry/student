...
import * as ConfigApi from '@/api/sales/config'
...
const formRef = ref()
const activeTab = ref<'afterSale' | 'delivery' | 'brokerage'>('afterSale')
const tabFormFields = {
  afterSale: ['afterSaleRefundReasons', 'afterSaleReturnReasons'],
  delivery: ['deliveryExpressFreePrice'],
  brokerage: [
...
const getCurrentTabFormFields = () => {
  if (activeTab.value === 'brokerage' && !formData.value.brokerageEnabled) {
    return []
  }
  return tabFormFields[activeTab.value]
}
const formData = ref({
  id: null,
...
const submitForm = async () => {
  ...
  try {
    const currentTabFormFields = getCurrentTabFormFields()
    if (currentTabFormFields.length > 0) {
      await formRef.value.validateField(currentTabFormFields)
    }
  } catch {
    return
  }
