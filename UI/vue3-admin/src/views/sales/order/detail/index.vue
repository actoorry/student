  ...
  <ContentWrap>
    ...
    <el-descriptions :column="4" title="收货信息">
      ...
      <div v-if="formData.deliveryType === DeliveryTypeEnum.EXPRESS.type">
        ...
        <!-- 电子面单信息 -->
        <el-descriptions-item v-if="formData.waybill" label="电子面单: ">
          <el-tag :type="waybillStatusType(formData.waybill.status)" size="small">{{ formData.waybill.statusName }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item v-if="formData.waybill" label="面单打印: ">
          <el-button
            v-if="formData.waybill.labelUrl"
            link
            type="primary"
            v-hasPermi="['sales:sales_electronic_waybill:reprint']"
            @click="openWaybillPrint"
          >
            <Icon icon="ep:printer" /> 预览/打印
          </el-button>
          <span v-else style="color: #909399">-</span>
        </el-descriptions-item>
        <el-descriptions-item v-if="formData.waybill && formData.waybill.failMessage" label="失败摘要: ">
          {{ formData.waybill.failMessage }}
        </el-descriptions-item>
...
import OrderUpdateAddressForm from '@/views/sales/order/form/OrderUpdateAddressForm.vue'
import OrderUpdatePriceForm from '@/views/sales/order/form/OrderUpdatePriceForm.vue'
import * as DeliveryExpressApi from '@/api/sales/delivery/express'
import { useTagsViewStore } from '@/store/modules/tagsView'
import { DeliveryTypeEnum, TradeOrderStatusEnum } from '@/utils/constants'
import * as DeliveryPickUpStoreApi from '@/api/sales/delivery/pickUpStore'
...
/** 电子面单状态 tag 类型：0-失败 1-有效 2-已作废 */
const waybillStatusType = (status?: number) => {
  switch (status) {
    case 1:
      return 'success'
    case 2:
...
/** 打开电子面单面单打印（浏览器本地打印） */
const openWaybillPrint = () => {
  if (formData.value.waybill?.labelUrl) {
    window.open(formData.value.waybill.labelUrl, '_blank')
  }
}
