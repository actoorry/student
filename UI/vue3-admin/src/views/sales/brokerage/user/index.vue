<template>
  <doc-alert title="【交易】分销返佣" url="https://doc.appap.vip/mall/trade-brokerage/" />
...
import * as BrokeragePartnerApi from '@/api/sales/brokerage/user'
import { checkPermi } from '@/utils/permission'
import { fenToYuanFormat } from '@/utils/formatter'
import BrokerageUserUpdateForm from '@/views/sales/brokerage/user/BrokerageUserUpdateForm.vue'
import BrokerageUserListDialog from '@/views/sales/brokerage/user/BrokerageUserListDialog.vue'
import BrokerageOrderListDialog from '@/views/sales/brokerage/user/BrokerageOrderListDialog.vue'
import BrokerageUserCreateForm from '@/views/sales/brokerage/user/BrokerageUserCreateForm.vue'
...
const getList = async () => {
  loading.value = true
  try {
    const data = await BrokeragePartnerApi.getBrokerageUserPage(queryParams)
...
const handleCommand = (command: string, row: BrokeragePartnerApi.BrokerageUserVO) => {
  switch (command) {
...
const openUpdateBindUserForm = (row: BrokeragePartnerApi.BrokerageUserVO) => {
...
/** 清除上级推广人 */
const handleClearBindUser = async (row: BrokeragePartnerApi.BrokerageUserVO) => {
  try {
    // 二次确认
    await message.confirm(`确认要清除"${row.nickname}"的上级推广人吗？`)
    // 发起修改
    await BrokeragePartnerApi.clearBindUser({ id: row.id })
...
/** 推广资格：开通/关闭 */
const handleBrokerageEnabledChange = async (row: BrokeragePartnerApi.BrokerageUserVO) => {
  try {
    // 二次确认
    const text = row.brokerageEnabled ? '开通' : '关闭'
    await message.confirm(`确认要${text}"${row.nickname}"的推广资格吗？`)
    // 发起修改
    await BrokeragePartnerApi.updateBrokerageEnabled({ id: row.id, enabled: row.brokerageEnabled })
