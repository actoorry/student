...
const submitForm = async () => {
  ...
  try {
    // 发起修改
    await BrokeragePartnerApi.createBrokerageUser(formData.value)
...
const userInfo = reactive<{
  bindUser: BrokeragePartnerApi.BrokerageUserVO | undefined
  user: BrokeragePartnerApi.BrokerageUserVO | undefined
}>({
  bindUser: undefined,
  user: undefined
})
const handleGetUser = async (id: any, userType: string) => {
  ...
  const user =
    userType === '推广员' ? await BrokeragePartnerApi.getBrokerageUser(id) : await PartnerApi.getPartner(id)
