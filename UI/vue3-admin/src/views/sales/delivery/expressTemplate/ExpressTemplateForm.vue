...
import * as DeliveryExpressTemplateApi from '@/api/sales/delivery/expressTemplate'
...
const formRules = reactive({
  ...
  charges: [{ validator: validateCharges, trigger: 'change' }],
  frees: [{ validator: validateFrees, trigger: 'change' }],
...
/** 校验计费规则：至少一条、区域非空、数值为正、区域互不重复 */
function validateCharges(_rule: any, value: any[], callback: (error?: Error) => void) {
  const charges = formData.value.charges
  if (!charges || charges.length === 0) {
    callback(new Error('至少需要一条计费规则'))
    return
  }
  const seen = new Set()
  for (const item of charges) {
    if (!item.areaIds || item.areaIds.length === 0) {
      callback(new Error('计费规则必须选择区域'))
      return
    }
    if (!(item.startCount > 0) || !(item.extraCount > 0) || !(item.startPrice > 0) || !(item.extraPrice > 0)) {
      callback(new Error('计费首续量、首续费必须大于 0'))
      return
    }
    const key = [...item.areaIds].sort().join(',')
    if (seen.has(key)) {
      callback(new Error('计费规则区域不能重复'))
      return
    }
    seen.add(key)
  }
...
/** 校验包邮规则：区域非空、数值为正、区域互不重复；可为空 */
function validateFrees(_rule: any, _value: any[], callback: (error?: Error) => void) {
  const frees = formData.value.frees
  ...
  for (const item of frees) {
    if (!item.areaIds || item.areaIds.length === 0) {
      callback(new Error('包邮规则必须选择区域'))
      return
    }
    if (!(item.freeCount > 0) || !(item.freePrice > 0)) {
      callback(new Error('包邮数量、金额必须大于 0'))
      return
    }
    const key = [...item.areaIds].sort().join(',')
    if (seen.has(key)) {
      callback(new Error('包邮规则区域不能重复'))
      return
    }
    seen.add(key)
  }
