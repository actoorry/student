        ...
        <template #header>
          <div class="flex items-center justify-between">
            ...
            <el-button
              type="primary"
              @click="onSubmit"
              v-hasPermi="['partner:sales-pool-config:update']"
            >
              保存
            </el-button>
          </div>
...
import * as PartnerPoolConfigApi from '@/api/partner/sales/poolConfig'
import { CardTitle } from '@/components/Card'

defineOptions({ name: 'PartnerPoolConfig' })
...
const getConfig = async () => {
  try {
    ...
    const data = await PartnerPoolConfigApi.getPartnerPoolConfig()
    if (data === null) {
      return
    }
    formData.value = data
...
const onSubmit = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as PartnerPoolConfigApi.PartnerPoolConfigVO
    await PartnerPoolConfigApi.savePartnerPoolConfig(data)
