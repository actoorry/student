<template>
  <doc-alert title="【通用】跟进记录、待办事项" url="https://doc.appap.vip/crm/follow-up/" />
...
const getCount = () => {
  PartnerApi.getTodayContactPartnerCount().then(
    (count) => (customerTodayContactCount.value = count)
  )
  PartnerApi.getPutPoolRemindPartnerCount().then(
    (count) => (customerPutPoolRemindCount.value = count)
  )
  PartnerApi.getFollowPartnerCount().then((count) => (customerFollowCount.value = count))
