...
const formRules = reactive({
  skuId: [{ required: true, message: '产品不能为空', trigger: 'blur' }],
  businessPrice: [{ required: true, message: '商机价格不能为空', trigger: 'blur' }],
  count: [{ required: true, message: '产品数量不能为空', trigger: 'blur' }]
})
const formRef = ref([]) // 表单 Ref
const productList = ref<SkuApi.SkuVO[]>([]) // 产品（SKU）列表
...
const handleAdd = () => {
  const row = {
    id: undefined,
    skuId: undefined,
    productUnit: undefined, // 产品单位名称
    productNo: undefined, // 产品条码
    productPrice: undefined, // 产品价格（分）
...
/** 处理产品变更 */
const onChangeProduct = (skuId, row) => {
  const sku = productList.value.find((item) => item.id === skuId)
  if (sku) {
    row.productUnit = sku.unitName
    row.productNo = sku.barCode
    row.productPrice = sku.price
    row.businessPrice = sku.price
  }
}
...
/** 初始化 */
onMounted(async () => {
  productList.value = await SkuApi.getSkuSimpleList()
})
</script>
