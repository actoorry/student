    ...
    >
      <el-form-item label="员工" prop="partnerId">
        ...
        <div v-if="selectedEmployee" class="mt-5px text-12px text-gray-500">
          工号：{{ selectedEmployee.employeeNo || '-' }} ｜ 部门：{{ selectedEmployee.deptName || '-' }}
        </div>
      </el-form-item>
      ...
      <el-form-item
        v-if="formData.recordType !== 'rural_support'"
        label="进修/培训名称"
        prop="practiceName"
      >
...
const open = async (type: string, id?: number, presetPartnerId?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await OutboundApi.getOutbound(id)
      formData.value = data
      if (data.partnerId && data.partnerName) {
        ...
        employeeOptions.value = [selectedEmployee.value]
      }
      areaIds.value = findAreaIdsByNames(data.province, data.city, data.county)
    } finally {
      formLoading.value = false
    }
  } else if (presetPartnerId) {
    // 从员工档案带 partnerId 跳转：自动预填员工
    formData.value.partnerId = presetPartnerId
    try {
      const page = await EmployeeApi.getEmployeePage({ pageNo: 1, pageSize: 1, partnerId: presetPartnerId })
      if (page.list && page.list.length > 0) {
        const emp = page.list[0]
