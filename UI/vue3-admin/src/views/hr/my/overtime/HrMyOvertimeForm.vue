    ...
    >
      ...
      <el-form-item label="姓名">
        ...
        <div v-if="selfEmployee" class="mt-5px text-12px text-gray-500">
          工号：{{ selfEmployee.employeeNo || '-' }} ｜ 所属科室：{{ selfEmployee.deptName || '-' }}
        </div>
      </el-form-item>
...
/** 职工我的加班登记 表单 */
defineOptions({ name: 'HrMyOvertimeForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await MyOvertimeApi.getMyOvertime(id)
      formData.value = {
        id: data.id,
...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'create') {
      await MyOvertimeApi.createMyOvertime(data)
      message.success(t('common.createSuccess'))
    } else {
      await MyOvertimeApi.updateMyOvertime(data)
